...
| `supabase-primary` (`.mcp.json`, added 2026-08-15) | **Adopted** — `@supabase/mcp-server-supabase`, project-ref-scoped to the authoritative Primary Supabase project (`vrvtsxexkiiiivlkdxzp`; full topology `MASTER_INTEGRATION_CATALOG.md §0`). Replaces reliance on an account-level claude.ai connector of the same name that dropped between sessions and was invisible to the repo — this entry is git-versioned and reproduces on any machine. **Requires a `SUPABASE_ACCESS_TOKEN` environment variable set locally by the owner (Supabase dashboard → Account → Access Tokens, using the `platplustours@gmail.com` account) — never committed, never pasted into chat/logs, never stored anywhere in this repository.** A new session must independently verify this server actually connects (its tools appear via `ToolSearch`) before treating Primary as reachable, and must re-verify the exact project ref before any schema-changing operation, per `MASTER_INTEGRATION_CATALOG.md §0`'s standing rules — a configured entry is not proof of a live connection. |
