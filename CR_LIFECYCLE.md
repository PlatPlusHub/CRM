...
- **Allocation** — a new identity is the first *unreserved* candidate at or above the sequence cursor plus one. The cursor is derived from history rather than from any maintained list: walking a ref's first-parent history newest-to-oldest, the first Change Request allocation event encountered — an added `changes/SPEC-<integer>-*.md` path, or a rename whose old and new paths carry different integer identities — supplies the cursor as its new-side identity, and the walk stops there. Because the walk stops at the newest event, an older anomaly can never become the cursor: `SPEC-1000`, `SPEC-1001`, and the transiently mistaken `SPEC-1002` are simply never reached. No repository-wide numeric maximum is consulted, so no synthetic-fixture enumeration has to be maintained — the list that used to live here drifted, and removing the need for it is the repair.
- **Reservation** — an identity is reserved if it appears in the ref's current tracked tree, or if it has *ever* appeared in that ref's reachable tracked history, in a blob's content or in an added or renamed path. Reservation is monotonic: deleting the occurrence never releases the identity. It is answered per candidate by Git itself — the current tree, one added/renamed path walk, and one content pickaxe, all anchored to the supplied authoritative ref and never to `--all`, whose reachability differs between clones. No ledger, registry, or allowlist exists.
- **Collision validation** — enforced mechanically by the Gate across the entire repository *including* the synthetic band: a newly added Change Request may never reuse an identifier that already appears in tracked filenames or tracked text (`SPEC_ID_ALREADY_USED`), may never reuse one reserved only by history (`SPEC_ID_HISTORICALLY_RESERVED`), and two newly added Change Requests may never introduce the same identifier in one diff (`DUPLICATE_NEW_SPEC_ID`).

Allocation decides which number to take; collision validation decides whether taking it is legal. Reservation serves both: the allocator *skips* a reserved candidate and advances to the next, while an agent that selects a reserved identity by hand is refused, and one that selects a free identity that is not the first unreserved candidate is refused as `SPEC_ID_NOT_NEXT`. A repository-wide textual maximum remains the wrong allocator precisely because it observes fixtures — and the collision check is right to observe them.

**Origination is a forward-only cutover (2026-09-18, `SPEC-196`).** A SPEC identity stays repository-wide and may still materialize as a migration, a pgTAP test, a master-plan entry, or any combination. What changed is which event *originates* a new sequence-advancing identity. Legacy identities that exist only outside `changes/` — `SPEC-147`, `SPEC-156`–`SPEC-159`, and the other sequence gaps — remain real engineering history and are never reinterpreted. From the activation marker below onward, only a Change Request allocation event advances the sequence; every other tracked occurrence reserves its number permanently but never advances it. This needs no new mechanism, because `AGENTS.md` §1 already confines creation to an Approved or In-Progress Write Scope and §2 already makes one Change Request one engineering step: an artifact carrying some other new number therefore does not originate an identity, it only burns one, and reservation prevents that number from ever being reused.

**Allocation enforcement activates on this declared marker, never on a diagnostic string.** The Gate reads it from a commit's first-parent copy of this file to decide whether that commit's allocation is judged, and requires it in its own working tree (`SPEC_ALLOCATION_MARKER_MISSING`) so deleting or renaming this authority fails loudly instead of silently disabling history enforcement. Commits whose first parent predates the marker are not retroactively judged for allocation; reservation is never gated by it.

SPEC Allocation Enforcement: 1

**Terminal-contract immutability activates on this declared boundary, never on a diagnostic string (2026-09-21, `CTRL-1`).** A `Complete` or `Cancelled` Change Request is historical and may not be modified, renamed or deleted. Whether that rule applied to a committed range was previously decided by searching the base ref's own copy of `scripts/check_agent_continuity.ps1` for the literal `HISTORICAL_CR_MUTATION`, so a behaviour-preserving rename landing inside the range made the evaluator look for a string the base could not contain, and the protection switched itself off in silence while every existing test stayed green. The boundary below is the **original** activation commit, and the Gate decides activation by Git ancestry against it: a ref at or after it is judged, a ref before it is not, which is exactly the truth table the previous form produced. It is deliberately NOT the `SPEC Allocation Enforcement` marker above, which first appears eight days later — adopting that would move the cutover forward and leave already-protected history unprotected. Missing or malformed evidence fails loudly (`HISTORICAL_ACTIVATION_MARKER_MISSING`, `HISTORICAL_ACTIVATION_MARKER_MALFORMED`); a boundary this repository does not contain is treated as ACTIVE, because being unable to prove a ref is pre-activation is not a reason to stop protecting it.

Historical CR Immutability Enforcement: 5d78aacd5335278c5b03edb0b3f969bd86e6b9c4
...
**Approval is admissible only on sufficient evidence (2026-09-18, `SPEC-196`).** A `Draft` carries a bounded `## Pre-Approval Evidence` section whose predicates resolve to `PASS`, `FAIL`, or `INDETERMINATE`, as defined by the `### Pre-Approval evidence sufficiency` rule in `ENGINEERING_METHOD.md`. Only PASS permits the human Draft-to-Approved decision; `FAIL` and `INDETERMINATE` both refuse the transition, and applicability is derived from repository evidence rather than from the contract's own `Change Class`, so a self-declared `Routine` cannot exempt control or governance work. The Gate evaluates this on the Draft-to-Approved transition alone, which is why terminal historical contracts are never retrofitted. The human remains the only actor that performs Approval — the mechanism decides admissibility, never the decision itself.
...
**Every range check now judges the commits that occurred, not the range's endpoints (2026-09-12, `SPEC-167`).** The preceding rule was applied to Status and to nothing else. `Diff-Records` computes a net `BASE..HEAD` diff, so a commit that violated an invariant and a later commit that undid it cancelled out and the violation was admitted having never been looked at — and `Status-Path` itself was walked for the resolved *governing* contract alone. Four classes are therefore defined as forbidden to have **occurred**, not merely forbidden to survive to `HEAD`, and `Validate-CommittedRange` audits each commit against its first parent to enforce them: a write outside the approved Write Scope; a widening of frozen authority, where the scope is read from the baseline rather than from the contract as it stood at the offending commit, so a commit cannot authorise its own writes; a modification to a contract after it became terminal inside that same range, which the BASE-only terminality test could not see; and an illegal Status transition in a contract that is not the governing one, so a valid corrective Change Request can never launder an invalid earlier one. This is deliberately not "every intermediate commit must independently be releasable" — a work-in-progress commit is legal, and final-state properties (the manifest pointer, completion prerequisites, the certification receipt) remain correctly judged at `HEAD` alone and are not re-checked per commit. Only history-sensitive invariants are audited, and no new error code was introduced: each raises the existing code with the offending commit appended after an `@`.
