...
> **Latest session report:** `history/phase-c-financial-capability-2026-08-28.md` — the SEC-1 inventory the directive required, and the defects it exposed. Every money table's policy NAMED a permission — `VIEW_FINANCIAL_DOCUMENTS`, a **read** permission, OR'd with a plain visibility test — so the effective rule was *"you may write money about anything you can see"*. **FIN-3 reproduced:** an employee holding no `RECORD_PAYMENT` inserted a 999,999 EGP payment by direct DML. `journal_entries` had required the real capability all along: **the ledger was guarded and the cash was not.** Six tables now charge exactly the permission their own RPC charges. **FIN-4:** `requested_by` on an approval request is derived, not accepted. Also corrects my own previous exposure figure (40 → 23 → and why counting policies that *name* a permission overstates enforcement), and records walking back into the plpgsql RECORD hazard SPEC-159-A had already documented. Predecessor `history/phase-c-role-journeys-2026-08-28.md` — the first role-by-role walk; RBAC-4, TASK-2, SEC-1 reproduced. Predecessor `history/phase-c-journey-branches-2026-08-28.md` — FIN-2, TASK-1 and TRANS-1. Predecessor `history/api-1-application-surface-and-employee-journey-2026-08-28.md` — ORVION stops being unreachable. Predecessor `history/wp-04e-storage-executor-and-api-reachability-2026-08-27.md` — the storage executor.
>
