...
> **Latest session report:** `history/wp-04e-storage-executor-and-api-reachability-2026-08-27.md` — the byte half of WP-04-D's split shipped as an Edge Function, chosen because the platform injects the service key itself so the route needs **zero new secrets**; **storage is PROVEN end to end for the first time** (36 assertions, real bytes, real HTTP). The sweep then found **API-1**: PostgREST exposes only `public`/`graphql_public`, so every one of ORVION's 137 `app.*` RPCs and all 8 `reporting` views are unreachable — 600 passing assertions had coexisted with a completely unreachable API, because every RPC test speaks SQL and not HTTP. Also closes **FND-1** (a failed storage action was hidden forever — PH8-1's shape) and **GRANT-1** (SPEC-124's class recurring for functions). Predecessor `history/wp-04d-retention-reconciliation-and-rbac-audit-2026-08-27.md` — retention and orphan reconciliation built on the fact that the database cannot delete an object, plus **RBAC-1**: no privilege revocation had ever been audited and direct-DML grants were unaudited too. Predecessor `history/wp-04c-document-storage-2026-08-27.md` — the storage provider decided on evidence, and SPP-1/SPP-2. Predecessor `history/wp-04b-payment-proof-lifecycle-2026-08-27.md` — the renewal-proof journey made executable end to end. Predecessor `history/wp-04a-document-write-integrity-2026-08-27.md` — the storage path became system-derived (DOC-1/DOC-3). Predecessor `history/employee-performance-and-financial-lineage-2026-08-27.md` (SPEC-159/159-A).
>
