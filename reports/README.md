...
> **Latest session report:** `history/phase-c-write-capability-2026-08-28.md` — FIN-3's rule applied to the whole schema wherever the evidence is unambiguous: for each unguarded table, find every `app.*` function that inserts into it and the permission it authorizes. **One RPC/one permission → guard it; two RPCs/two permissions → guard the union, which is exactly what the code already permits; no RPC or an RPC that authorizes nothing → left to SEC-1 rather than guessed.** Nine tables guarded. `approval_requests` is INSERT-only because `review_finance_approval` updates it and finance does **not** hold `CREATE_BOOKING_ITEM` — checked before writing, not after breaking FIN-2. SEC-1's residue fell 40 (mis-measured) → 23 → **13**, and the thirteen are now classified individually. Also records a test of mine that would have been vacuous — a trainee denial on a table the trainee cannot even see. Predecessor `history/phase-c-financial-capability-2026-08-28.md` — the ledger was guarded and the cash was not (FIN-3/FIN-4). Predecessor `history/phase-c-role-journeys-2026-08-28.md` — RBAC-4, TASK-2, SEC-1 reproduced. Predecessor `history/phase-c-journey-branches-2026-08-28.md` — FIN-2, TASK-1, TRANS-1. Predecessor `history/api-1-application-surface-and-employee-journey-2026-08-28.md` — ORVION stops being unreachable.
>
