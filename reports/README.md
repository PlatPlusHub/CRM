...
> **Latest session report:** `history/phase-c-journey-branches-2026-08-28.md` — Phase C's first pass walked the branches an agency actually works (refund, cancellation, complaint, service request, supplier payment, approvals, conversations, tasks, document versioning) over HTTP as real users, and found **two dead capabilities**: **FIN-2** — an employee could not REQUEST finance approval because the guard treated writing `finance_approval_status_code='pending'` as APPROVING, so only the approver could ask themselves; and **TASK-1** — an employee could create and complete a task but not START one. Fixing TASK-1 did NOT fix it, which is the important finding: transition permissions live in TWO places (`advance_*` inline lists and `app.status_transitions`, which the trigger reads), so the first fix drifted them apart. Both corrected, and the diff is now a permanent guard (**TRANS-1**). Also records **RLS-1** — a read permission confers write authority on 11 tables, latent rather than live. Predecessor `history/api-1-application-surface-and-employee-journey-2026-08-28.md` — ORVION stops being unreachable; 71 endpoints classified rather than blindly wrapped, and the employee revenue journey proven over HTTP. Predecessor `history/wp-04e-storage-executor-and-api-reachability-2026-08-27.md` — the storage executor and API-1's discovery. Predecessor `history/wp-04d-retention-reconciliation-and-rbac-audit-2026-08-27.md` — retention/reconciliation and RBAC-1. Predecessor `history/wp-04c-document-storage-2026-08-27.md` — the storage provider decided on evidence.
>
