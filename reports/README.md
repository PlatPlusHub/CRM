...
> **Latest session report:** `history/phase-c-role-journeys-2026-08-28.md` — the first role-by-role walk ORVION has had (senior employee, branch manager, department manager, finance, CEO, owner, platform), which found **RBAC-4**: a department manager could see **zero** bookings in the department they manage while every employee under them saw them all — `VIEW_DEPARTMENT_RECORDS` was granted to employee and senior_employee and not to the manager. Also closes **TASK-2** (a started task emitted `task_assigned`, a code `assign_task` already owns, inflating assignment counts; canon 26 defines no start event, so it now emits none rather than a falsehood) and **reproduces SEC-1**: a role holding only `VIEW_ALL_BRANCHES` renamed a booking, retitled a task and inserted a customer — 40 tables accept direct INSERT with no capability check anywhere. RLS-1 merged into SEC-1; the exposure is now measured and capped. Predecessor `history/phase-c-journey-branches-2026-08-28.md` — the journey branches, FIN-2, TASK-1 and TRANS-1. Predecessor `history/api-1-application-surface-and-employee-journey-2026-08-28.md` — ORVION stops being unreachable. Predecessor `history/wp-04e-storage-executor-and-api-reachability-2026-08-27.md` — the storage executor. Predecessor `history/wp-04d-retention-reconciliation-and-rbac-audit-2026-08-27.md` — retention and RBAC-1.
>
