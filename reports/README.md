...
> **Latest session report:** `history/session-2026-09-01-attr2.md` — **ATTR-2: the hand that records is not always the hand that acted.** Six actor columns were accepted from the caller on the direct-DML door while their only RPC already recorded the caller and offered no parameter to name anyone else. Reproduced with live positive and negative controls, including a finance_manager who genuinely holds RECORD_PAYMENT recording a payment as received by the employee while ATTR-1's trigger corrected `created_by` in the same statement, and an employee holding nothing rewriting `payments.received_by` with no capability at all. Two of the six were invisible to the detector meant to find them, because they carry an actor without a `_by` suffix (MEAS-4) — replaced with a structural predicate, counterexample-tested both ways. `payments.received_by` was decided by measurement rather than by the suffix; `payments.verified_by` was deliberately NOT derived and is recorded as VERIFY-1. Migration `202607059300`, deployed.
>
