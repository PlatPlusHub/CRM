...
| PH8-1 | A `pending` delivery is never recoverable — an interrupted n8n run strands its conversion permanently | operations | High | R | 8 | A | ➕ | OPEN | pending | phase8-workflow-review-2026-08-17 | 08-17 | 08-17 |
...
- **Candidate fix (design-ready, not implemented — needs owner approval as it adds a recovery RPC):** a stale-pending reaper — either a `claim`-time reclaim of `pending` rows older than a timeout, or a separate `app.requeue_stale_conversion_deliveries(interval)`. The existing `attempt_number` and retry-ceiling machinery already support it; no schema change is required. Precedent for the pattern: this is the standard outbox lease/visibility-timeout, the same problem SQS solves with visibility timeouts.
