...
| **Production database deployment** | ✅ **CERTIFIED (synchronized)** | Owner-directed deployment 2026-08-10: SPEC-120's 3 migrations applied to `orvion-core` via the Supabase MCP connection and re-verified live against the project itself (not assumed) — `catalog_values=569`, `subscription_plans=3`, all 4 refund events present, `customer_merged` absent. Residual note: the migrations landed under MCP-assigned versions (`20260810172943`/`55`/`06`), not local file versions (`202607049700`/`800`/`900`) — content verified identical; both idempotent, but a future `supabase db push` will not see them as version-matching. |
