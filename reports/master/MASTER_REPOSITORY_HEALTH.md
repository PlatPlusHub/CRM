...
Last measured: 2026-08-10 (Owner-directed deployment — after the handover audit corrected a false same-day claim, SPEC-120's three migrations were deployed to `orvion-core` via the Supabase MCP connection and re-verified live against the project: catalog/subscription-plan data now matches local exactly. Local and production both report 569 catalog values, 67 catalog types, 3 subscription plans. Repository worktree and GitHub `main` match at `25694e8`). Values marked ⚙ are computable today (grep/script); values marked 👁 require the link-checker/governance-lint tooling to be exact (currently best-effort).
