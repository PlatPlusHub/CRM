...
`public.users` is the row that decides who exists in a tenant and whether they can act. Batch 6 Slice 12 selected it by measurement (`scripts/batch6_select_target.ps1`: Exposure **15**, the highest surface still `NOT-RECORDED`), and two defects were reproduced against the clean local baseline at `c28c526`:
...
None. `SPEC-193`, `SPEC-194` and `SPEC-195` are terminal `Cancelled` contracts covering the same slice; this Change Request does not supersede, extend, amend or edit any of them, and requires no status change on any file. Their relationship to this work — including one inherited design detail this contract measured to be wrong — is recorded in Notes.

## Write Scope

- `changes/SPEC-197-membership-authority-and-audit.md`
- `supabase/migrations/20260919120000_membership_authority_and_audit.sql`
...
| migration set 218 → 219, latest → `20260919120000`, ledger fingerprint | `reports/evidence/primary-ledger-evidence.json`; `_ORVION_CANONICAL/manifest.md` `Live state:` | WRITE | Both rewritten from a post-deployment Primary read (GUARD-1), never from a repository list |
...
| SPEC-198's `Last Completed` sentence compressed to reclaim Check 5 budget | `_ORVION_CANONICAL/manifest.md` current-state readers, Check 5, the cold-boot sequence | WRITE | Semantic-preserving: the compressed sentence still names the applicability repair and the `NOT APPLICABLE` outcome. Swept for other consumers of that text — `git grep` finds the SPEC-198 summary quoted nowhere else, and `ai-map.json` copies it by value and is regenerated, so no consumer loses a required current-state fact |
...
| Check 5 — `_ORVION_CANONICAL/manifest.md` inside its 7000-character budget | BEFORE_IMPLEMENTATION | NONE | NONE | NONE |
| All profiles green with a fresh certification receipt | BEFORE_COMPLETION | NONE | NONE | NONE |

**MANIFEST-1, and why it is authorized here rather than discovered at the Approve commit.** Measured on `90558c9`: `_ORVION_CANONICAL/manifest.md` is **6971** characters against Check 5's **7000** budget, leaving **29**. Approving this contract replaces `Active Change Request: None` with a pointer to this file, which costs **46** more characters and yields **7017** — so the Approve transition is *structurally guaranteed* to trip `MANIFEST BLOAT` unless the same commit reclaims the space. Reproduced in a disposable transition, and the repair proved there: compressing the SPEC-198 `Last Completed` entry brought the file to **6882** with the pointer set and Check 5 green.

The budget is never raised and Check 5 is never weakened — `ENGINEERING_METHOD.md` states the rule as trim, not exempt. The repair is therefore a **semantic-preserving compression of current-state prose and nothing else**: the SPEC-198 `Last Completed` sentence is shortened while still naming what SPEC-198 changed (applicability derived from a CONTROL profile alone, now from any non-`REPOSITORY` profile, a control surface, `supabase/functions/**` or a declared irreversible action) and its user-visible outcome (`NOT APPLICABLE` instead of a silent `PASS`). No other manifest field is touched by that edit, no current-state fact is deleted, and `_ORVION_CANONICAL/manifest.md` is already in this contract's Write Scope, so the edit is authorized rather than an undocumented side-edit. It is declared `BEFORE_IMPLEMENTATION` because it must hold at the Approve commit, which precedes Step 1; it opens no red window, because the trim and the pointer land in the same commit.

**Check 10 carries no red window in this contract, and that is deliberate.** It is the invariant that made `SPEC-195` unsatisfiable: that contract created a session report, moved `reports/README.md`'s live pointer in an early step, moved the matching manifest `Narrative:` only after the Primary gate, and placed between them a gate admitting exactly three failure classes — so a fourth, `STALE POINTER`, was structurally guaranteed at precisely the moment it was forbidden. This contract writes **no session report** and touches neither `reports/README.md` nor the manifest `Narrative:` field. The disposition row cites this Change Request as its evidence pointer instead, which Check 22 resolves through the same repository-wide basename index it uses for session reports (verified by reading the check). The pairing therefore never disagrees, and the class of failure is removed rather than sequenced around.

Every other red window opens and closes **on the same side** of the Step-10 pre-deploy gate. In particular the `CHECK 6b` catalog pin is repaired at Step 7, *before* that gate demands `ALL CHECKS PASSED`, rather than after the deployment it guards. The three failure classes still standing at Step 10 are exactly the undeployed-migration state seen from three angles; they are named and bounded there, and Step 12 closes all three after the deployment that is their cause.
...
Derived mechanically, not chosen. **At the time this identity was allocated** — the Draft commit `67e6092`, whose first parent was `c28c526` — the first-parent sequence cursor was **196** (`SPEC-196`, Complete); the candidate was `cursor + 1` = **197**; and `SPEC-197` was unreserved by all three reservation queries: absent from the current tree, from every added or renamed path in reachable history, and from the content pickaxe over that history. No skip was required and the first free candidate was itself. No repository numeric maximum, manual cursor, fixture allowlist or hard-coded expectation was used, and no future identifier is written into tracked text by this contract.

That allocation is a historical event and is not re-derived. The cursor has since advanced to **198** because `SPEC-198` was allocated and completed after this Draft was committed; that does not reallocate, renumber or invalidate `SPEC-197`, whose identity is fixed by its own committed allocation and is permanently reserved by the Reservation Rule. Current legality therefore rests on that committed allocation plus reservation, never on the present value of the cursor, and it is re-proved mechanically by the Gate on every run rather than asserted here.
...
2. **Check:** a file matching `supabase/migrations/20260919120000_*.sql` exists. If present, record Already Applied. Otherwise create `supabase/migrations/20260919120000_membership_authority_and_audit.sql` containing exactly these five parts and nothing else:
...
10. **Check:** the working tree contains exactly one migration absent from the ledger recorded in `reports/evidence/primary-ledger-evidence.json`. Perform the pre-deploy readiness gate. Every item must hold and each must be recorded in the Execution Log with its measured result; the first that does not hold is a STOP, and nothing is deployed while it stands:
   ...
   - `pwsh -NoProfile -File scripts/check_repository_consistency.ps1` is run and its result recorded. It is EXPECTED to be non-green here. Exactly **three** failure classes are admissible, all of them the same bounded undeployed state seen from three angles, and each must be named and matched to that cause: `MIGRATION STATE DRIFT`; `SUITE FIGURE DRIFT`; and **RECOVER-1 / Check 19**, admissible only when the guard's `only in repository` set contains exactly one entry and that entry is `20260919120000_membership_authority_and_audit`, its `only on Primary` set is empty, and the evidence file is present, valid, complete, internally consistent and attributable to this history. **Any failure outside those three classes is a STOP**, explicitly including any `Check 10` result, any `PRIMARY HAS … MIGRATION(S) THE REPOSITORY DOES NOT` report, more than one repository-only migration, or any evidence-file attribution failure.

11. **Check:** `reports/evidence/primary-ledger-evidence.json`'s `ledger` array contains an entry beginning `20260919120000`. If present, record Already Applied. Otherwise perform the Primary synchronization, in this order and no other, stopping at the first step that does not hold:
   ...
   - (c) prove from that reading that `20260919120000` is ABSENT and that the pre-deployment ledger equals the currently recorded evidence — same `migration_count` and same `ledger_fingerprint`;
   - (d) re-confirm (a), (b) and (c) immediately before deploying, then apply ONLY `supabase/migrations/20260919120000_membership_authority_and_audit.sql` through the connector's migration-apply call, and nothing else;
   - (e) if the connector assigns its own migration version on apply, normalise that version in Primary's ledger to the repository filename version `20260919120000`, the practice already recorded for `20260909060754` and `20260909114354`;
...
12. **Check:** `_ORVION_CANONICAL/manifest.md`'s `Batch 6 surface coverage` line reads `13 of 77`. If it does, record Already Applied. Otherwise, AFTER Step 11 has completed, update that file by measurement only:
   ...
   - update `Last Completed` and `Next capability` together per `CR_LIFECYCLE.md §9` so that SPEC-197 becomes `Last Completed` and `Next capability` names Batch 6 Slice 13 ranked by `scripts/batch6_select_target.ps1`;
...
- [ ] `supabase/migrations/20260919120000_membership_authority_and_audit.sql` exists and registers those three codes in `public.catalog_values` under `event_type` at `sort_order` 932, 933 and 934.
...
- [ ] `supabase/tests/31_access_revocation_test.sql` clears `request.jwt.claims` immediately before its administrative deactivation, carrying a `SPEC-197` comment explaining why; its plan count, every assertion's SQL, expected value and description, and the claim it sets afterwards are unchanged from their state at the start of this Change Request.
...
- [ ] `reports/master/MASTER_SURFACE_DISPOSITION.md` records `users` as `AUDITED-OPEN` / `ADVERSARIAL` citing `SPEC-197-membership-authority-and-audit`, and its Coverage summary agrees with its rows.
...
- [ ] `_ORVION_CANONICAL/manifest.md` is inside Check 5's 7000-character budget at every commit of this contract's lifecycle, including the Approve commit that adds the `Active Change Request` pointer, and its compressed SPEC-198 `Last Completed` entry still names the applicability repair and the `NOT APPLICABLE` outcome.
...
- [ ] `reports/evidence/primary-ledger-evidence.json` names `project_ref` `vrvtsxexkiiiivlkdxzp`, contains `20260919120000` in its `ledger` array, and its `migration_count` and `ledger_fingerprint` are consistent with that array.
...
- [ ] `_ORVION_CANONICAL/manifest.md` records Batch 6 coverage as `13 of 77`, names SPEC-197 as `Last Completed`, and its `Live state:` sentence publishes the catalog pair as `71/621` with every mutable figure written from a post-deployment measurement.
...
**What this contract owes to three cancelled predecessors, and what it does not inherit.** `SPEC-193`, `SPEC-194` and `SPEC-195` carried a similar engineering design for this same slice and were each cancelled on owner authority for contract defects, never for a falsified finding. Nothing here is inherited on their authority: USR-1 and USR-2 were re-reproduced against `c28c526`, the blast radius was re-measured by running the suite against a local prototype, and the consumer sweep was re-derived with `git grep`. That re-measurement paid for itself three times. First, `SPEC-195` froze `sort_order` 166/167/168 for these same three event codes; those values are already held by `refund_approved`, `refund_rejected` and `refund_cancelled`, so this contract uses 932/933/934. Second, `SPEC-195` included `supabase/tests/75_human_identity_family_test.sql` in Write Scope; measurement shows that file passes unchanged, so it is in Out of Scope here instead. Third, the consumer sweep found `reports/history/session-2026-09-09-ten-decision-closure.md` carries the same three counts this change moves — it is classified UNAFFECTED because it is a dated historical record, and editing it would falsify history.
