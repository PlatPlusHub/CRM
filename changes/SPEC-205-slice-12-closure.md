...
### 2026-09-20 — CANCELLED. Its frozen Additional Verification cannot return 0.

From `In Progress` the legal exits are `Complete` and `Cancelled`. `Complete` requires a
`LOCAL_CERTIFY: READY` receipt, `-Finish` runs every Additional Verification command that is not
already mandatory, and `Invoke-Verification` throws on any non-zero exit. This contract froze
`pwsh -NoProfile -File scripts/check_database_parity.ps1`, which measurement proves cannot return 0
in this repository — 2 bare, 1 fully supplied. `Additional Verification` is a frozen section, so the
line cannot be corrected in place. The contract is therefore unsatisfiable by its own text, which is
the same class of defect that ended `SPEC-200` and `SPEC-203`, and the third time the frozen text
rather than the engineering has been what failed.

**What this contract nevertheless achieved, and what is kept:** the blocker that cancelled
`SPEC-203` is REPAIRED and committed at `89bba0b`. `reports/master/MASTER_API_CONTRACT.md` was
regenerated by its own generator against a clean repository-derived database, the delta was proven
to be exactly one line before being accepted, and `check_database_parity.ps1` Check L3 now reports
`MASTER_API_CONTRACT.md matches the live surface`. That work is done and does not need repeating.

**What the successor must NOT repeat:** it must not name the bare parity command as a verification
it has to pass. Check L3 is proved by an Implementation Step and pinned by an Acceptance Criterion —
which Review re-verifies against live state — rather than by a Finish gate that an unrelated,
already-recorded finding holds shut. That is not weakening the measuring layer: nothing about
`check_database_parity.ps1` is edited, and its verdict is still recorded.

**The larger finding is escalated rather than absorbed.** `PAR-5` is not the low-severity reporting
nuisance it was first classified as. Because `Get-ProfileEvidence` lists the bare parity command as
MANDATORY for the `DATABASE` profile, and because that command cannot return 0 while the structural
hash includes platform-managed `service_role` grants, **no `DATABASE`-profile contract can currently
be certified at all**. That is a control-plane blocker on every future database slice, and it
belongs to a control change with its own contract — not to this recovery, whose priority is closing
Slice 12.

No file outside Write Scope was touched. Primary was not contacted by this contract in any way, and
Secondary never has been.
