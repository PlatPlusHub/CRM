...
None. Two terminal `Cancelled` contracts are carried in Write Scope **for range publishability only**, because the net diff of the publishable range contains their cancellation commits; this contract must not modify a byte of either, and an Acceptance Criterion pins both.

`changes/SPEC-197-membership-authority-and-audit.md` is terminal and is **not** superseded here: its Slice-12 objective is untouched and returns as its own legally allocated successor. `changes/SPEC-200-draft-baseline-and-approval-replay.md` is terminal and carried the identical CTRL-2A/CTRL-2B engineering; it was cancelled for a defect in its own frozen text — an unsatisfiable Mutation Obligation (iv) — and not for any falsified finding. This contract re-issues that engineering verbatim from a preserved patch, with exactly the three mutation obligations measurement actually supports.
...
| `changes/SPEC-200-...md` appears in the publishable range's net diff (its Draft, Approve, In Progress and Cancel commits) | the endpoint Write Scope check | WRITE | Same terminal-carriage mechanism as `SPEC-197`, proven by simulating the exact range before Approval rather than assumed; pinned byte-identical after `cf38cc4` by an Acceptance Criterion |
...
Mutation Obligation: **exactly three**, because measurement found exactly three load-bearing predicates, and each is killed by a scenario no other guard can rescue — (i) removing the `$baselineStatus -ne 'Draft'` gate must make the legal Draft-revision range fail again; (ii) inverting that gate so the block runs only for a `Draft` baseline must make a post-Approval **Acceptance Criteria text** mutation pass, which is the isolating scenario because `Validate-Checklist` runs at the endpoint alone while `Validate-CommittedRange` re-compares frozen *sections* per commit and would otherwise kill this mutation for the wrong reason; (iii) removing the range replay must make the invalid intermediate Approval pass. Each must record `applied=True`, a green pristine run and a red mutant run, and no mutation may be credited to a guard other than the one it names.

A fourth mutation is deliberately **not** required, and this is a measured finding rather than an omission. Making the replay read the endpoint contract instead of the approving commit's own bytes cannot change any verdict on a legal range: `Evaluate-PreApprovalEvidence` reads only `Write Scope`, the `Pre-Approval Evidence` section and `Implementation Steps`, all three of which are `FrozenSections`, and `Get-ControlSurface` is a hard-coded list so derived applicability cannot drift either. Proved with a three-way discriminator whose only post-Approval edit was a permitted Runtime-Checkpoint advance — pristine `BLOCKED / APPROVAL_EVIDENCE`, replay-disabled `READY` (the kill that proves the path executes), endpoint-bytes `BLOCKED` with the identical subject. That predicate is therefore recorded as PROVEN REDUNDANCY, and inventing a fourth mutation to preserve a count would be manufacturing evidence.
