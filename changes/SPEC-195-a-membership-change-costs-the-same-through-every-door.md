...
Supersedes `changes/SPEC-194-a-membership-change-costs-the-same-through-every-door.md`, which is **Cancelled** (owner-authorized, 2026-09-17), and inherits its objective, engineering design and Write Scope unchanged. SPEC-194 reached the mandatory smoke test during execution and stopped BEFORE Primary: `scripts/verify_database.sql` pins `catalog_values` at 618 and this migration legitimately seeds three, giving 621, while that file sat in SPEC-194's Out of Scope. Everything before the smoke passed — Pass A 118/1944, six HTTP suites, Pass B 118/1944, declared equals executed, test 31 10/10 with its semantics intact, test 118 in full with its mutation control, exactly one undeployed migration, zero only-on-Primary, no out-of-scope path — and Primary was never touched. SPEC-191 and SPEC-193 were cancelled earlier in the same slice for the same CLASS of failure, never for the engineering. This contract adds only what a mechanically derived dependency-closure audit proved, and that audit is recorded in Notes so the next contract does not rediscover it.
...
- `supabase/tests/31_access_revocation_test.sql` — its lines 95-101, where this file ALREADY clears the JWT claim before an administrative write for exactly this reason, and states why in its own comment
...
6. **Check:** `supabase/tests/75_human_identity_family_test.sql` contains the string `SPEC-195` in assertion 11's description. If present, record Already Applied. Otherwise amend assertion 11's description only — leaving its SQL and its expected SQLSTATE `42501` unchanged — to record that since SPEC-195 the refusal is delivered by `users_guard_membership_authority` before RLS's `WITH CHECK` is reached, and that the RPC remains the only reachable claim path. Change no other assertion in the file.

7. **Check:** `supabase/tests/31_access_revocation_test.sql` contains the string `SPEC-195` in the comment immediately above its administrative deactivation. If present, record Already Applied. Otherwise insert `select set_config('request.jwt.claims', null, true);` immediately before the bare `update public.users set is_active = false where id = '32000000-0000-0000-0000-000000000011';` that follows its `reset role;`, carrying a comment headed `SPEC-195` recording that the file's own `reset role` clears the SQL role but not the JWT claim, that `users_guard_membership_authority` derives the caller from `auth.uid()` rather than from the role, and that this is therefore the same stale-claim fixture defect already corrected in `35_subscription_write_gate_test.sql` and in `118_membership_authority_and_audit_test.sql`. Change nothing else in the file: not its plan count, not any assertion's SQL, expected value or description, and not the JWT claim it sets afterwards at its own line 77. This is a FIXTURE repair only — the production trigger, `auth.uid()` semantics, the MFA requirement and the assertion's intended access-revocation behaviour are all untouched, and the file must still prove that a deactivated employee resolves to no tenant, no user, sees nothing and holds nothing.

8. **Check:** `reports/master/MASTER_GAP_REGISTER.md` contains a row whose first cell is `USR-1`. If present, record Already Applied. Otherwise add rows `USR-1` and `USR-2` in the file's existing table format marked resolved by this migration; update the existing `IDENT-2` row to resolved, stating it was closed by the same emitter rather than separately; and add one OPEN row, identified as `USR-3`, recording the measured, deliberately unrepaired step-up gap on `user_permission_grants` and `user_branch_assignments` as separate surfaces. The identifier is named here rather than left to the executing agent, because SPEC-194's equivalent step mandated the row and its content without naming it, and an unnamed register row is an ambiguity a later reviewer has to resolve by guessing.
...
11. **Check:** `scripts/verify_database.sql` contains the string `621 catalog_values`. If present, record Already Applied. Otherwise, AFTER a clean `npx supabase db reset` that includes this contract's migration, INDEPENDENTLY re-count `select count(*) from public.catalog_values` and `select count(*) from public.catalog_types` against that reset database, and only then update the file's catalog pins from the measured values. The count is not incremented from 618 on paper: it is read, and the read must show exactly **621** — 618 plus this contract's three `event_type` rows and nothing else. Prove the difference is exactly those three codes by naming them. Update every living place in that file that states the current pin, which measurement of the file shows to be three: the baseline comment at its head (`… 71 catalog types, 618 catalog values …`), the `CHECK 6b` assertion itself (`if n <> 618 then raise exception 'CHECK 6b FAILED: expected 618 catalog_values, found %'`), and the closing `ALL CHECKS PASSED` notice which embeds `71/618 catalog`. Extend that check's existing causal-history comment with one line recording this slice's `611 -> 618 -> 621` step, in the voice the file already uses. **Do not weaken the assertion**: it stays an exact equality on a literal, never an inequality, a range or a tolerance — the file's own comment states the rule, *"The pin moves with the seed; the check is not weakened."* `CHECK 6a`'s `catalog_types` pin of **71** must be left untouched, and the measurement must CONFIRM it is still 71 rather than assume it: this contract seeds `catalog_values` rows under an existing `catalog_type_code` and adds no catalog type. Causally prove the repair in both directions: the pre-change file FAILS against the new clean-reset state with `CHECK 6b FAILED: expected 618 catalog_values, found 621`, and the updated file PASSES against that same state. Change no other check in the file.

12. **Check:** `reports/evidence/primary-ledger-evidence.json`'s `ledger` array contains an entry beginning `20260917120000`. If present, record Already Applied. Otherwise perform the Primary synchronization, in this order and no other, and stop at the first step that does not hold:
...
13. **Check:** `_ORVION_CANONICAL/manifest.md`'s `Batch 6 surface coverage` line reads `13 of 77`. If it does, record Already Applied. Otherwise, AFTER local verification has passed and AFTER the Primary synchronization step has completed, update that file as follows and by measurement only:
   - set the `Batch 6 surface coverage` line to `13 of 77`;
   - update `Last Completed` and `Next capability` together per `CR_LIFECYCLE.md §9` so that SPEC-195 becomes `Last Completed` and `Next capability` names Batch 6 Slice 13 ranked by `scripts/batch6_select_target.ps1`;
...
- [ ] `_ORVION_CANONICAL/manifest.md` records Batch 6 coverage as `13 of 77` and names SPEC-195 as `Last Completed`.
...
- [ ] `supabase/tests/31_access_revocation_test.sql` clears `request.jwt.claims` immediately before its administrative deactivation, carrying a `SPEC-195` comment explaining why; its plan count, every assertion's SQL, expected value and description, and the claim it sets afterwards are unchanged from their state at the start of this Change Request.
...
- [ ] `scripts/verify_database.sql` pins `catalog_values` at **621** in all three places that state it — the head comment, `CHECK 6b`, and the `ALL CHECKS PASSED` notice — as an exact equality on a literal, with its `CHECK 6a` `catalog_types` pin still **71** and every other check unchanged.
...
- [ ] `scripts/verify_database.sql` completes with `ALL CHECKS PASSED` during the pre-deploy readiness gate, and again after Primary synchronization.
- [ ] `_ORVION_CANONICAL/manifest.md`'s `Live state:` sentence publishes the catalog pair as `71/621`, each side written from a post-deployment measurement rather than carried over.
- [ ] `reports/master/MASTER_GAP_REGISTER.md`'s new OPEN sibling row is identified as `USR-3`.
...
Three contracts in this slice were blocked by one root cause: Write Scope derived from the implementation files and from the guards already tripped, rather than closed over every executable validator and authoritative derived fact the change affects. This matrix is the mechanical derivation, taken from a `git grep` sweep of the tracked tree for every literal that encodes each changed fact. Every candidate ends classified; there are no unknowns.

| Changed fact | Consumer found by sweep | Classification |
|---|---|---|
| migration set 218 → 219, latest → `20260917120000`, ledger fingerprint | `_ORVION_CANONICAL/manifest.md` L32; `reports/evidence/primary-ledger-evidence.json` (`migration_count`, `ledger_fingerprint`, `ledger`) | **WRITE REQUIRED** — both in scope |
| | Check 9, Check 19/RECOVER-1, `check_database_parity.ps1`, Migration CI, DATABASE profile | **READ/VERIFY ONLY** — measured: none of the three validator scripts hard-codes any changed value; all derive them |
| pgTAP population 117/1915 → 118/1944 | `_ORVION_CANONICAL/manifest.md` L32 only | **WRITE REQUIRED** — in scope |
| | Check 15, Pass A / Pass B | **READ/VERIFY ONLY** — Check 15 derives the total from literal `plan(N)` sums |
| **`catalog_values` 618 → 621** | **`scripts/verify_database.sql`** L8 head comment, **L130 `CHECK 6b`**, L178 `ALL CHECKS PASSED` notice embedding `71/618` | **WRITE REQUIRED** — the dependency SPEC-194 missed; newly added to Write Scope |
| | `_ORVION_CANONICAL/manifest.md` L32 (`71/618 catalog`) | **WRITE REQUIRED** — in scope, remeasured by Step 13 |
| `catalog_types` (unchanged at 71) | `scripts/verify_database.sql` L117 `CHECK 6a`, L8, L178 | **READ/VERIFY ONLY** — this contract seeds rows under an existing type and adds none; the 71 is confirmed, never edited |
| event vocabulary +3 codes | `_ORVION_CANONICAL/27_event_catalog.md` | **WRITE REQUIRED** — in scope |
| | `_ORVION_CANONICAL/25_catalog_registry.md` | **NOT AFFECTED** — measured: it carries `## security_event_type` and `## offline_conversion_event_type` and has **no `## event_type` section**. Canon 27 is the sole owner of the event vocabulary; adding 25 would create the second owner `record_event`'s own error message argues against |
| `users` → `AUDITED`/`ADVERSARIAL`, coverage 12 → 13 of 77 | `reports/master/MASTER_SURFACE_DISPOSITION.md` (row, derived counts, `All 12`); `_ORVION_CANONICAL/manifest.md` L34; `reports/README.md` pointer; the history report; test 118's `-- ATTACK-CLASSES:` header that Check 24 demands | **WRITE REQUIRED** — all in scope |
| findings rows USR-1 / USR-2 / USR-3 / IDENT-2 | `reports/master/MASTER_GAP_REGISTER.md` | **WRITE REQUIRED** — in scope; `USR-3` is now named by Step 8 rather than chosen during execution |
| `app.create_tenant_user` body loses its `record_event` | `reports/master/MASTER_API_CONTRACT.md` (GENERATED, "do not hand-edit") | **NOT AFFECTED** — proven cell by cell against its `endpoint/args/returns/sec/permission/inserts/updates/raises/http` header: `raises` 1 → 1, `inserts` 1 → 1, `updates` 0 → 0, signature identical, still `invoker`. No cell moves |
| any | `.github/workflows/migration-ci.yml` | **NOT AFFECTED** — its `push:` paths already include `supabase/migrations/**`, `supabase/tests/**` and `scripts/verify_database.sql`, and it already executes the smoke. It observes the change; it does not encode it |
| any | `.github/workflows/repository-consistency.yml` | **NOT AFFECTED** — its path lists already cover `supabase/migrations/**` and `supabase/tests/**`, which is what Check 20 requires |
| any | `scripts/check_repository_consistency.ps1`, `scripts/check_primary_ledger.ps1`, `scripts/check_database_parity.ps1` | **NOT AFFECTED** — measured to hard-code none of `618`, `621`, `1915`, `1944`, `218` or the latest migration id. They are correctly detecting the state and must not be changed to admit it |
| any | `repository-index.md`, `reports/master/MASTER_REPOSITORY_HEALTH.md` | **NOT AFFECTED** — swept: neither encodes any changed fact |

**The generalisable rule this slice earned, stated so a fourth repeat is not needed:** a migration's Write Scope must close over *(i)* the manifest `Live state:` sentence, *(ii)* `ai-map.json`, *(iii)* `reports/evidence/primary-ledger-evidence.json`, and *(iv)* **every executable invariant that pins a count the migration moves** — which for a `catalog_values` seed means `scripts/verify_database.sql`. Validators that merely *derive* the fact are read-only and must never be edited to admit a change.

**The test-31 repair is this file's own precedent, not a new judgement.** `31_access_revocation_test.sql` lines 95–101 ALREADY clear the JWT claim before an administrative write, added when RBAC-1 put `emit_role_change` on `user_role_assignments`, and its comment states the reasoning verbatim: *"`reset role` changes the database role but leaves `request.jwt.claims` set, so `auth.uid()` still resolves … That hybrid (postgres role, employee identity) cannot occur in production."* Step 7 applies that established pattern to the earlier `public.users` write, which the same class of trigger now governs.
