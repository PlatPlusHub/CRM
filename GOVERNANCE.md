...
| `reports/evidence/primary-ledger-evidence.json` | recorded reading of Primary's migration ledger (RECOVER-1) | Living — **machine-read; never hand-edit a count or hash**, because Check 18 recomputes both from the ledger array and a hand edit is detected as self-inconsistent | that a Primary reading is on record for this migration set and this history | whoever changes the migration set, in the same commit, from a live `supabase-primary` MCP read |
