...
- v1.16 (2026-09-11) — owner-authorized deterministic Agent Control Plane: unrestricted repository
  ...
  remain authoritative; `check_agent_continuity.ps1` only validates/extracts/dispatches them.
- v1.15 (2026-09-10) — synchronized the workstation artifact registry with the verified one-click
  entry path: `workstation.cmd` now dispatches directly to the authoritative provisioner rather than
  the optional maintenance menu. Registry-only correction; no SSOT reassignment.
- v1.14 (2026-09-05) — **DISP-1**: added the **per-surface audit disposition** SSOT row (§2) for `reports/master/MASTER_SURFACE_DISPOSITION.md`, Batch 6's exit criterion **EC-1**. Registered at creation rather than after the fact, which is the whole point of **GOV-2**: `MASTER_API_CONTRACT.md` was Living-Authoritative for weeks with no §2 row, and a Master without one is a duplicate-authority risk waiting to be discovered. The row states the boundary in both directions — the file holds **only** a disposition and an assurance level per surface, every other cell is a pointer, and the surface SET is derived from `supabase/migrations/**` rather than maintained, so it cannot drift from the schema (**Check 22**). It also records that **`EXHAUSTIVE` is deliberately not a per-surface value**: exhaustive adversarial audit is a claim about the programme and must never be assertable one table at a time. §5's Master row stopped restating a count (it read "15") and now points at §2, which is the same delete-don't-refresh answer **GOV-5** established. MINOR: new SSOT row for a new concept; no reassignment.
- v1.13 (2026-09-05) — **STALE-1**: added §6 rule 9, *freshness metadata is a claim about the document and the document is its own evidence*. A current-state document's `Last updated:` / `Last measured:` line may never be older than the newest date its own body carries. Added because the class had been found and hand-repaired **four times** in three different Master documents — 2026-07-14 (`MASTER_ARCHITECTURE_DECISIONS`), 2026-07-15 and 2026-08-29 (`MASTER_GAP_REGISTER`), 2026-08-29 and 2026-09-05 (`MASTER_EXECUTION_PLAN`) — and had never produced a check, so each repair restored the field and left the mechanism that lets it rot. The rule deliberately keeps the field **hand-written** (it carries an annotation git cannot supply) and makes only its **consistency** mechanical: Docs-21 / Check 21, CI-gated. **This version line is itself in that check's scope** — §15 already requires it to move with the top changelog entry, and a rule whose own home is unguarded is decorative. MINOR: new drift-prevention rule in an existing section; no SSOT reassignment, no restatement of the enforced-invariant registry (`MASTER_REPOSITORY_HEALTH.md §2b`).
- v1.12 (2026-09-03) — **RECOVER-1**: added the **Primary ledger evidence** SSOT row (§2) + registry entry (§5). `reports/evidence/primary-ledger-evidence.json` is the single home for the recorded reading of Primary's migration ledger, and Check 19 of `check_repository_consistency.ps1` fails closed when it is absent, self-inconsistent, bound to a non-ancestor commit, or disagreeing with `supabase/migrations/**`. Added because Primary ran four migrations the repository did not have for a day while every guard reported CLEAN — not because a guard lied, but because **the repository recorded nowhere whether Primary had ever been read at this HEAD**, making the question unanswerable and therefore unasked. Owner-directed (RECOVER-1 closure package). MINOR: new SSOT row for a new artifact class; no reassignment. The row states the evidence class explicitly — a RECORDED reading, never a live one — so it cannot be quoted for more than it measures.
- v1.11 (2026-08-29) — **AUD-03**: the findings-SSOT row (§2) now states the plan↔register boundary explicitly instead of leaving it to be inferred from "never restate the finding". The register owns a finding's **status and evidence**; `MASTER_EXECUTION_PLAN.md` legitimately narrates what a package fixed and why it was sequenced where it was, because a package narrative cannot be written without describing findings and that history is what makes the programme continuable. The harmful half — a status the register disagrees with — is now **measured** by Check 2's new cross-Master pass (**AUD-04**) rather than left to prose discipline. PATCH/MINOR: clarifies an existing row's boundary and moves an assertion into a measurement; no SSOT reassignment.
- v1.10 (2026-08-29) — added the **API capability contract** SSOT row (§2) + registry entry (§5): `reports/master/MASTER_API_CONTRACT.md` is the single home for the client-facing surface (endpoints, tenant-reachable tables, reporting views, permissions, HTTP-evidence coverage). It is the first **auto-generated** Master, so both entries state that it must never be hand-edited and that `check_database_parity.ps1` Check L3 enforces that by regenerating and diffing. Registered during the pre-Phase-10 program reconciliation, which found it had been Living-Authoritative since 2026-08-29 with **no SSOT row at all** — the duplicate-authority risk §2 exists to prevent (**GOV-2**). §5's Master count corrected 14 → 15. MINOR: new SSOT row for a new concept; no reassignment.
...
| Agent runtime / boot routing | `AGENTS.md §3–§4` (conduct) + `scripts/check_agent_continuity.ps1 -Boot` (deterministic extractor; owns no truth) | `README.md`, tool pointers and `ai-map.json` route to the command; manifest + active CR remain the underlying authorities |
...
| Accepted architectural findings/gaps — a finding's **STATUS and EVIDENCE** | `reports/master/MASTER_GAP_REGISTER.md` | ALL other Masters reference finding **IDs**; none may state a finding's status. **Boundary (AUD-03, 2026-08-29):** `MASTER_EXECUTION_PLAN.md` legitimately *narrates* what a package fixed and why it was sequenced where it was — a package narrative cannot be written without describing findings, and that history is what makes the programme continuable. What it must never do is assign a **status** the register disagrees with. That is the harmful half of the duplication, and it is now **measured**, not trusted: `check_repository_consistency.ps1` Check 2 compares finding status ACROSS every `reports/master/*.md` |
| Finding evidence trail (9 stages) | `reports/evidence/ARCHITECTURE_PROOF_LOG.md` | — |
| **Recorded reading of Primary's migration ledger** (RECOVER-1) | `reports/evidence/primary-ledger-evidence.json` — the full `version_name` set, its fingerprint, the project ref, the read timestamp, the exact read query, and the commit it was read at. **Evidence class: a RECORDED reading, never a live one** — it cannot prove the ledger came from Primary rather than from the repository (GUARD-1); it proves that *some* Primary reading is on record for this exact migration set and this history. Enforced fail-closed by `check_repository_consistency.ps1` **Check 19** (→ `check_primary_ledger.ps1`), mutation-proven by `test_primary_ledger_guard.ps1`. Refresh it by reading Primary via the `supabase-primary` MCP whenever the migration set changes | `manifest.md` publishes the derived live-state figures and must agree; `check_database_parity.ps1` cross-checks a supplied `-PrimaryFingerprint` against it and fails on contradiction |
...
| **Client API capability contract** (endpoints, tenant-reachable tables, reporting views, permissions, HTTP-evidence coverage) | `reports/master/MASTER_API_CONTRACT.md` — **AUTO-GENERATED from the live database; do not hand-edit.** Regenerate via `scripts/generate-api-contract.ps1`; `check_database_parity.ps1` **Check L3** regenerates and diffs it, so a hand edit fails the guard | canon + `MASTER_GAP_REGISTER.md` own the *rules*; this file states the *surface* those rules produce and restates none of them |
| **Per-surface audit disposition** — what assurance each database surface has been given, by which session, and what remains (Batch 6 **EC-1**) | `reports/master/MASTER_SURFACE_DISPOSITION.md` — one row per `public` table, holding **only** the disposition and the assurance level; every other cell is a pointer. The surface SET is **derived** from `supabase/migrations/**`, never maintained by hand, and `check_repository_consistency.ps1` **Check 22** fails on a phantom row, a missing row, an off-vocabulary value, or a session/finding pointer that does not resolve. **It owns no findings** (the register does), **no narrative** (the named session report does), **no schema** (the catalog does) and **no test inventory** (`supabase/tests/**` does) | `MASTER_EXECUTION_PLAN.md` states Batch 6's exit criteria and references this file for the count; the register owns every finding it cites; **`EXHAUSTIVE` is deliberately not a value here** — exhaustive adversarial audit is a claim about the programme, never about one row |
...
| AI memory / handoff | active CR Runtime Checkpoint + manifest + Git + executable evidence | chat/client memory is cache only — never the sole home of an operational fact |
...
Normal CR execution and routine checkpoints do not require a session report. CR state, Runtime Checkpoint, manifest, Git and tests carry continuity. A durable report is earned for forensic/audit work, substantial research, a durable architectural decision, a phase/capability boundary, or evidence that has no stronger existing representation.
...
| `workstation.cmd` (root) → `.workstation/prepare.ps1` | One-click workstation preparation | Living | nothing — thin pass-through to the authoritative provisioner | keep thin (no setup logic) |
...
| `reports/master/MASTER_*.md` — **each has its own §2 row, which is the list** | findings/plan/blueprint/health/integration contracts/API surface/**surface disposition** | Living — **except `MASTER_API_CONTRACT.md`, which is Auto-generated: never hand-edit, regenerate** | see §2 rows | every review |
...
| `reports/evidence/primary-ledger-evidence.json` | recorded reading of Primary's migration ledger (RECOVER-1) | Living — **machine-read; never hand-edit a count or hash**, because Check 19 recomputes both from the ledger array and a hand edit is detected as self-inconsistent | that a Primary reading is on record for this migration set and this history | whoever changes the migration set, in the same commit, from a live `supabase-primary` MCP read |
...
9. **Freshness metadata is a CLAIM about the document, and the document is its own evidence (STALE-1, 2026-09-05).** *(Appended as rule 9 rather than inserted above rule 8: `§6.8` is cited by name in seven places as the one-authority rule, so renumbering it would have broken all seven — rule 2's ID-stability discipline, applied to this file's own numbering.)* A current-state document's `Last updated:` / `Last measured:` line must never be **older than the newest date its own body carries** — the moment dated content is added, that line is either updated in the same edit or it is false. Three rules make the field cheap enough to keep true rather than expensive enough to skip: (a) **the field stays hand-written**, because it carries a written account of what changed and why, which no generator can supply — git knows only that a file was touched, so generating it would either destroy the annotation or advertise a typo fix as a substantive update; (b) **the invariant is semantic, never age-based** — a document nobody has touched is not stale, so "must equal today" is the wrong rule and would make the check permanently red and therefore ignored; (c) **the update APPENDS** — write the new dated line and demote the old one to `Previously:`, because these headers are cumulative history, not a mutable field. **Historical reports are exempt by construction, not by exemption**: they carry `Date:`, are immutable (rule 3), and make no freshness claim to contradict. Mechanically enforced as **Docs-21 / Check 21**; the enforced-invariant registry is `MASTER_REPOSITORY_HEALTH.md §2b` and this rule is not restated there.
...
- **How to start:** run `pwsh -NoProfile -File scripts/check_agent_continuity.ps1 -Boot`; follow its exact action and context pointers.
...
- **Where you may read/write:** reading/search is repository-wide. Create/modify/delete only paths in the active Approved/In-Progress CR's exact Write Scope. Never edit HISTORICAL-IMMUTABLE or generated artifacts by hand.
...
- **Never preload governance performatively:** Boot routes the smallest starting context; read this file or any other authority when its pointer or evidence need applies.
...
|---|---|---|---|
| **Governance-lint hook** (custom, in `scripts/`) | enforces: every report has a class header; no finding ID has conflicting status across Masters; `manifest.md Active CR` cleared after Complete | **High** | **IMPLEMENTED (2026-07-15/16):** `scripts/check_repository_consistency.ps1` + the `Repository Consistency` CI workflow; also in the doc definition-of-done (AGENTS §5, run before commit). **All three problems in the column to the left are now enforced** — the class-header lint, the finding-status comparison, and (2026-09-02, COLD-3) the `Active CR` check, which validates the pointer against the named CR's own `## Status` rather than only against the literal string `None`. **The invariant list and the remaining-extensions list that used to be restated here are removed, not refreshed (2026-09-02):** both had gone stale — this cell said "four invariants" against eighteen, and still listed the `Active CR` lint as pending after it shipped. The enforced-invariant registry is `reports/master/MASTER_REPOSITORY_HEALTH.md §2b` and is the only place that list lives (§6.8). Still outstanding: the full external link-check. |
