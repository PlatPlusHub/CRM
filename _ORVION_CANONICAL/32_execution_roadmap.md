...
- **API reachability** — 71 RPC endpoints + 8 reporting views are live and generated into `MASTER_API_CONTRACT.md`; **API-3** tracks those still lacking HTTP evidence, which is a coverage debt, not a reachability blocker. The current count is read from the generated contract, never restated here (**REG-2**).
