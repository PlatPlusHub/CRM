...
Active Change Request: changes/SPEC-208-parity-evidence.md
...
Last Completed: **`PAR-5` closed — structural parity now compares the grant model ORVION owns, certified by SPEC-207 (2026-09-20).** It hashed `service_role`, whose baseline is a platform default ACL no migration here sets, so the one live local↔Primary comparison was a standing false red that masked real drift. Narrowed to `authenticated`/`anon`: all ten surfaces now agree, `check_database_parity.ps1` returns exit 0 for the first time, and real drift in those roles still flips it to DRIFT.
...
Next capability: **Batch 6 Slice 13**, ranked by `scripts/batch6_select_target.ps1` — but it ships a migration and **`PAR-6` blocks `DATABASE`-profile certification**; that owner decision comes first. **The control-plane optimization chapter is CLOSED** — reopening it requires newly earned evidence (a real incident, a false green or false red, a measured material cost, or a proven safety/governance gap), never a further search for theoretical optimizations. Standing facts: Ruleset 22950574 requires `orvion-acceptance` on the default branch with `non_fast_forward`, `deletion` and zero bypass; candidates are published by `scripts/publish_candidate.ps1`, which proves the committed range first and replaces a rejected preflight candidate only under a caller-pinned lease; `main` is promoted by pushing the exact accepted SHA. Migration CI was compared on one candidate by SPEC-203: it agreed exactly.
