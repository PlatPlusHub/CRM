...
Last Completed: **SPEC-129 — tenant-qualified foreign keys (2026-08-21)**, from the foundation gate. Every tenant-scoped FK permitted a cross-tenant reference: a booking in Tenant B referencing Tenant A's customer was accepted, because a single-column FK checks existence, not ownership, and RLS's WITH CHECK validates only the new row's own tenant_id. Measured scale: 189 FKs, 50 child tables, 26 parents, zero composite keys. Migration `202607050600` adds `UNIQUE (tenant_id, id)` to every parent and re-points all 189 FKs at it. Found by a failing assertion in **SPEC-128** (migration `202607050500`), which separately resolved ATTR-1 by snapshotting conversion identity at creation so customer edits and retries cannot rewrite historical truth — with the `claim_conversion_deliveries` signature unchanged, so the n8n contract did not move. Guards `14_tenant_qualified_fk_test.sql`, `13_conversion_identity_snapshot_test.sql`; suite 14 files / 67 tests PASS. Detail: `changes/SPEC-129-tenant-qualified-foreign-keys.md`, `changes/SPEC-128-conversion-identity-snapshot.md`.
