...
Active Change Request: `changes/SPEC-166-gate-exit-status-is-its-own.md`
...
Last Completed: **SPEC-165 range completion of a contract created in range, Complete (2026-09-12).** A Change Request may now be created and completed inside one pushed range, judged by the Status path actually committed; the base-existence test that refused that legal history is gone, and cases 112/113 still reject a contract created as `Complete` or taken `Draft -> Complete`. Closes the CI rejection of the `SPEC-164` push, which itself bound `Complete` to a fresh `-Finish` certification receipt, made `DATABASE` execute the `ENGINEERING_METHOD.md §4` protocol, and made `-Certify` prove the EXPECTED workflow set against the exact SHA. Control suite 94 → 120 assertions. Product and schema untouched.
