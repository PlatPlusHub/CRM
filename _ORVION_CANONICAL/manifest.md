...
Live state: **repository, local stack and Primary `vrvtsxexkiiiivlkdxzp` all at 186 migrations** (latest `202607059700`), ledger `8de18286d9f215585c7d41b56f5a8c18`, full-function-surface hash `1c27a181022140ac63baa772354db54b` (254 functions) and structural-surface hash `ee4c9cda6ce4396f4f750bab9904cd8b` (3,393 objects across ten surfaces) identical on both; **75 tables**; 71/601 catalog; **8 reporting views**; **72 client RPCs**. Every figure re-read live from BOTH environments 2026-09-02; **parity PROVEN, Primary values read FROM Primary** (GUARD-1). Suite **91 files / 1273 assertions**, plus **395 end-to-end HTTP assertions** across six scripts, Pass A = Pass B. Primary holds **zero business rows**.

Active Change Request: None.

Open owner decisions — **IDs only; every definition, its evidence and its status live in `MASTER_GAP_REGISTER.md`, enforced by Check 11.** Every ID on THIS line is read as an open decision; a decided one leaves the line, and the register's OWNER-1 row holds the 2026-09-01 reconciliation that removed eighteen of them. Genuinely open: **QUO-4** · **SUP-4** (what counts against a supplier credit limit: the limit is currency-less, the payable is per-currency) · **RET-1** (legal-retention period; counsel required) · **FIN-7** + **VOID-1** + **VERIFY-1** (unimplemented finance capabilities whose columns already exist; an Invoice state machine would be NEW canon) · **TRANS-1** (classify the 13 ungoverned status columns against canon 26 first) · **DELIV-1** + **PH8-2** (model the existing lease semantics before adding states) · **PLAN-1** (the three "Limited" ceilings canon leaves undefined — a pricing decision) · **DOC-LC-2** · **DOC-LC-3** · **CANON-26-1** · **LIC-1** (external dependency).

Last Completed: **SUP-3 — supplier credit management is its own permission (2026-09-02, `202607059700`), deployed, parity re-proven.** OWNER DECISION, implemented the day it was given: `finance_manager` must be able to set `suppliers.credit_limit_amount`, and the capability must be **independently grantable**, not a side effect of ASSIGN_SUPPLIER. **`MANAGE_SUPPLIER_CREDIT`** minted per the VIEW_DEPARTMENT_RECORDS precedent — owner/ceo/finance_manager, plan-gated `finance_lite` to match the READ. Minting alone would have silently failed: finance_manager holds no ASSIGN_SUPPLIER, so `guard_write_capability` now recognises a credit-ONLY write by row image. The three permissions are orthogonal both ways, proven on both doors; SUP-2 stays closed. Grant→revoke→regrant proven behaviourally. **SUP-4 recorded, NOT invented** — the ceiling enforces nothing and ORVION has no authoritative definition of what counts against it (currency-less limit vs per-currency payable). Narrative: `session-2026-09-02-supplier-credit-permission.md`.
