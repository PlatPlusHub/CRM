...
Next capability: (current objective) **Build and verify the n8n offline-conversion workflow via the n8n MCP server — start a FRESH Claude Code session first.** n8n's official MCP server is configured and OAuth-authenticated (`claude mcp list` → `n8n: ✔ Connected`), granting workflow create/update/execute and execution-read; **no paid n8n upgrade is required** (available on all editions). Its tools do not load in the session that added it — a fresh session picks them up automatically, and that is the single remaining prerequisite. **Before building, read `MASTER_INTEGRATION_CATALOG.md §2a` — a pre-build review found three defects in the draft node spec, one high-severity** (acking a `validateOnly` run as success permanently marks real conversions delivered that were never delivered). The workflow does **not** yet exist in n8n (an earlier claim that it did was false and is corrected in `§4`). `orvion_integration` login is independently verified; the Google OAuth client and n8n Postgres credential remain owner-reported only. Also open from the same review: PH8-1 (unrecoverable `pending` deliveries — must be fixed before the workflow runs unattended on its schedule), PH8-2/PH8-3 (owner decisions), PH8-4 (informational) in `MASTER_GAP_REGISTER.md`. The real-GCLID test stays owner/data-dependent until genuine ad-click traffic exists — no fabricated conversions or click IDs. Other remaining roadmap items stay either **owner-gated** — subscription lifecycle (C4/C5 business policy); HR (owner scheduling); Phase-10 comms *implementation* (Meta accounts/verification) — or **evidence-gated** (Phase 9 Tier B on measured cost; composite indexes A2 on volume; Google OAuth verification at pre-go-live). **One autonomous option remains open:** the Phase-10 Meta-ecosystem Learn-Before-Designing research + communications-domain Design Challenge.
