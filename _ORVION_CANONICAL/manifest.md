...
Last Completed: **The §5b impact/consumer capability (2026-09-05) — `scripts/impact.ps1`, a query and not a guard.** It answers *what consumes, parses, derives from or validates this structure* from the live catalog (function bodies, policy expressions, trigger definitions **including arguments** — the blind spot `pg_depend` and MEAS-1 share) and from the repository, labels every section with its evidence class, and refuses (exit 2) under `-RequireFresh` when local is not the repository (PAR-1b). Validated in both directions against known examples; **no migration, no schema change, PRIMARY UNPROVEN and not claimed.** Before it, **GOV-18, CUST-3, VOID-1 and RET-1 (2026-09-04)** — four owner-directed capabilities, each verified through the full `§5a` protocol and deployed to Primary, with fourteen real defects caught by the guards before any of it shipped. Narrative: `session-2026-09-05-engineering-awareness-layer.md`.
