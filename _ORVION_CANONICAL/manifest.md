...
Last Completed: **META-1 — the guards were treated as hypotheses (2026-09-01), no migration.** A **14-mutation battery** against the repository guard: **nine caught, five MISSED**, and the five were one blind spot — the manifest publishes current-state FIGURES nothing owned. The serious one: **the structural-surface hash could be corrupted to zeros and BOTH guards stayed CLEAN** (the repository guard opens no database; the parity guard never read the manifest) — the very number a cold start reads for "is Primary synchronized?". Fixed with two DERIVED checks: **Check 15** (suite figures vs the test files' own `plan(N)`; RPC count vs the generated contract) and **Check L5** (published hashes vs the live database, compared to LOCAL not to supplied Primary values — GUARD-1). Two misses left unmechanised WITH REASONS. **Check 15 caught real unplanted drift on its first run** (1154 vs 1156). **Test 85 proven load-bearing** by dropping its trigger — the write succeeded — now a permanent PAR-4 pair. Narrative: `session-2026-09-01-meta-review.md`.
