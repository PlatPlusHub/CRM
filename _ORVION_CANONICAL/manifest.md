...
Last Completed: **WP-03 / SPEC-152 — subscription state enforcement (2026-08-26, `202607053100`), EARNED/CLOSED.** `app.plan_allows` inverted the owner's rule at both ends: `read_only` permitted writes, and `suspended`/`expired`/`cancelled` denied reads. State moved out of `plan_allows` into `app.enforce_subscription_write_gate`, a BEFORE INSERT/UPDATE/DELETE trigger on **42** tables — chosen because 32 of 89 write policies never call `has_permission`, so a gate there would have missed most CRM work. Reads survive every restricted state; no subscription fails CLOSED for writes. Fifteen exemptions keep the reactivation and provisioning paths open. Proven in `supabase/tests/35_subscription_write_gate_test.sql` (24 assertions). Detail: `reports/history/wp-03-subscription-state-enforcement-2026-08-26.md`.

Next capability: **Foundation Hardening — Zero-Known-Debt (owner directive 2026-08-24). The n8n workflow stays GATED.** Suite 35 files / 331 assertions; 120 migrations; 71 permissions. Security posture (each proven by a named test): branch/department/assigned read scope, RBAC + finance write authority, plan gating, lifecycle + archive enforced against direct DML, employee financial privacy, document privacy, **audit-spine write integrity (WP-00)**. Remaining, in order:

1. **Post-WP-03 discovery pass** (3 named questions in the WP-03 report's NEXT STEP), then **WP-01 — creation events**: four registered `*_created` types have an executable producer but never fire — `customer_created`, `lead_created`, `passenger_created`, `trusted_device_created`.
