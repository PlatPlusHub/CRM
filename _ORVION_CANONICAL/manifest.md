...
Next capability: **Foundation Completion Programme, in progress (owner directive 2026-08-24). The n8n workflow stays GATED behind it.** STEP 1 (AUDIT-3) and the escalation it exposed are done — SPEC-137 (read-scope model), SPEC-138 (RBAC write authority) and SPEC-139 (employee financial privacy) SPEC-140 (assignment-history integrity + `reassign_lead`) and SPEC-141 (CAT-5/CAT-6 + the plan matrix) — 108 migrations, suite 25 files / 202 tests. All three owner visibility rules (branch isolation, department continuity, financial privacy) are implemented and proven behaviourally. **Both are applied LOCALLY ONLY: the `supabase-primary` MCP disconnected and needs re-authorization in an interactive session, so Primary is behind by 7 migrations and parity is unverified — deploy and re-verify before treating the Foundation as frozen.** Remaining, in order:
