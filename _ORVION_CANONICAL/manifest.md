...
Last Completed: **Batch-6 table-by-table audit, care/conversation slice (2026-08-31), `202607059000` — DEPLOYED, parity CLEAN.** Four of the five tables came back clean (checked, not counted: `conversations` lacks two guards its siblings have and has nothing for them to apply to); `quotation_items` failed twice. **QUO-2 (Medium)**: `app.add_quotation_item` refuses a line on a non-draft quotation and the table did not — a 7,777 line landed on a quotation the customer already had, and an existing line went 10,000 → 1 through the half that has NO RPC at all. **QUO-3**: the table had no CHECK constraints whatsoever, storing −5000 and quantity 0. **QUO-4 is an OWNER DECISION**, reproduced and deliberately not fixed. **ATTR-2**: widening `83_actor_attribution_test.sql` assertion 22 from a name list to every `_by` column found EIGHT more caller-supplied actor columns — the same detector failure three times in one day, now pinned so it fails in either direction. Narrative: `session-2026-08-31-batch6-care.md`.
