...
Active Change Request: `changes/SPEC-184-candidate-publication-proves-its-range.md`
...
Last Completed: **SPEC-183 preflight skips the duplicate mutation suite, Complete (2026-09-15).** `Agent Control` re-ran the argument-free `test_agent_continuity.ps1` on `orvion-preflight`, where `ORVION Acceptance` already runs it on the same candidate SHA. That one step is now conditioned on the ref; its Gate, which resolves `github.event.before -> github.sha`, still executes there. Assertions 162-163 fix that only that step is conditioned.
