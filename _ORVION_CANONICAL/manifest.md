...
Active Change Request: `changes/SPEC-172-acceptance-carries-the-evidence-it-replaces.md`
...
Last Completed: **SPEC-171 a cancellation is not a governing act, Complete (2026-09-12).** `SPEC-170` made `Cancelled` committable but left one Complete beside one Cancelled resolving as `AMBIGUOUS_GOVERNING_CR` — the ordinary shape of every cancellation, since closing a contract requires carrying it inside another's Write Scope. That reached `main` and failed there (`cde4f06`, left visible, not rewritten). `Resolve-Contract` now resolves to the single `Complete` contract; an abandoned contract authorised nothing, and Write Scope still comes from the resolved contract alone. Two Completes or two Cancelleds still fail closed. Assertion 129 had asserted the broken rule and passed, because test and code shared one unexamined conclusion. Suite 137 → 140 assertions.
