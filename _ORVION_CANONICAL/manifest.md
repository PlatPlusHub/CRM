...
Last Completed: **SPEC-130 — enforce entity references (2026-08-21)**, from the foundation gate. The polymorphic `related_entity_type`/`related_entity_id` link on tasks, notifications and approval_requests had no vocabulary, no referential integrity and no tenant safety — `'BoOkInG'` pointing at a non-existent booking was accepted. Migration `202607050700` adds the `related_entity_type` catalog family and `app.enforce_entity_reference`, enforcing pairing, vocabulary, existence and same-tenant. The polymorphic shape was kept deliberately: canon 29 states "any business entity", so the `document_links` typed-FK alternative was rejected as contradicting domain intent. Catalog baseline moved to 68/583. Guard `supabase/tests/15_entity_reference_test.sql`; suite 15 files / 75 tests PASS. Detail: `changes/SPEC-130-enforce-entity-references.md`.
