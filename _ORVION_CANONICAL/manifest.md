...
Active Change Request: `changes/SPEC-183-preflight-skips-the-duplicate-mutation-suite.md`
...
Last Completed: **SPEC-178 repository dates are culture-invariant, Complete (2026-09-14).** Repository Consistency Checks 12, 21 and 23 parsed the repository's `yyyy-MM-dd` contract with a `$null` format provider, which resolves to `CurrentCulture`; under a non-Gregorian calendar every parse failed and each check asserted success over zero records. All four parse sites and five diagnostic renders now use an invariant culture. Future-date suite 5 -> 9 assertions.
