...
Last Completed: **SPEC-134 - master-data write paths (2026-08-21)**, resolving RPC-2. Four employee-facing entities had no permission of their own; the answer was determined from canon without inventing a key or amending canon 28: contact methods and notes are Customer-aggregate sub-records under CREATE_CUSTOMER (the create_passenger precedent), suppliers use ASSIGN_SUPPLIER, campaigns use MANAGE_MARKETING_CAMPAIGN. Migration `202607051100` adds six RPCs with duplicate prevention where duplicates are accidental and none where repetition is a business fact, and canonicalizes contact data on contact methods and suppliers. Tables with no RPC insert path 35 -> 26. Suite 19 files / 121 tests PASS. Detail: `changes/SPEC-134-master-data-write-paths.md`.
