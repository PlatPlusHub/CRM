...
Last Completed: **Batch 6 slice 10 -- `invoices` (2026-09-08).** `202607062000` closed **INVOICE-1** (High -- **an amount is a PAIR and `guard_financial_capability` named one half of it**: an actor with NO financial permission ran `set currency_code = 'USD'` and `customer_balance` read 50,000 **USD** for 50,000 EGP, `total_amount` never moving), **INVOICE-2** (High -- customer, booking, number, date all writable at the table door), **INVOICE-3**, **INVOICE-4** (born `paid`, or `voided` with no reason), **INVOICE-5** (a FULLY PAID invoice's total raised 50,000 -> 90,000, a gap `record_payment` can never close), **INVOICE-6** (archive attribution caller-supplied on **thirteen** tables) and **INVOICE-7** (one renamed invoice stopped `create_invoice` tenant-wide). **68 assertion 7's column-scoped principle stands** -- only its consequence set moved, where a reader was measured. **Method finding: MECHANISM SUBSTITUTION** -- a new guard silently took over an older assertion's refusal, same SQLSTATE. **ENTRY-1 opened**: ten tables have a governed transition machine and an ungoverned entry. Narrative: `session-2026-09-08-batch6-slice10-invoices.md`.
