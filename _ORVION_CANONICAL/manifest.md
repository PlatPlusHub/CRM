...
Next capability: **Foundation Hardening — Zero-Known-Debt (owner directive 2026-08-24). The n8n workflow stays GATED.** Fourteen CRs delivered (SPEC-137…150; see `changes/` and git log). Suite 33 files / 296 tests; 117 migrations; 71 permissions. Security posture: branch/department/assigned read scope, RBAC + finance write authority, plan gating in `has_permission`, lifecycle transitions and archiving both enforced against direct DML, employee financial privacy, document privacy. Remaining, in order:
1. **Primary deployment and verification** — blocked on interactive re-authorization; nothing else closes the freeze without it.
