...
Last Completed: **SPEC-132 - customer-service write paths (2026-08-21)**, second delivery of the RPC-1 programme. `conversations`, `conversation_messages`, `complaints` and `service_requests` had no RPC at all; migration `202607050900` adds seven governed RPCs implementing the canonical state machines, taking unenforced permissions from 34 to **27**. Also adds `supabase/tests/17_employee_walkthrough_test.sql`, which drives the real RPCs as a real authenticated user through auth.uid() to current_tenant_id() to authorize(), and proves a trainee genuinely cannot resolve a complaint. Suite 17 files / 102 tests PASS. **RPC-1 remains OPEN** (10 of 37 closed); new **RPC-2** isolates four entities with no permission defined at all. Detail: `changes/SPEC-132-customer-service-write-paths.md`.
