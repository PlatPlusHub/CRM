...
Live state: **repository, local stack and Primary `vrvtsxexkiiiivlkdxzp` all at 185 migrations** (latest `202607059600`), ledger `28f56ec055d53d42f90f7fef4a7317fd`, full-function-surface hash `393f55466f87d66a7713bdf715bf4080` (254 functions) and structural-surface hash `5ecc348211a3d9a87f6b1b266f6ea8d4` (3,393 objects across ten surfaces) identical on both; **75 tables**; 71/601 catalog; **8 reporting views**; **72 client RPCs**. Every figure re-read live from BOTH environments 2026-09-02; **parity PROVEN, Primary values read FROM Primary** (GUARD-1). Suite **90 files / 1247 assertions**, plus **390 end-to-end HTTP assertions** across six scripts, Pass A = Pass B. Primary holds **zero business rows**.

Active Change Request: None.

Open owner decisions — **IDs only; every definition, its evidence and its status live in `MASTER_GAP_REGISTER.md`, enforced by Check 11.** Every ID on THIS line is read as an open decision; a decided one leaves the line, and the register's OWNER-1 row holds the 2026-09-01 reconciliation that removed eighteen of them. Genuinely open: **QUO-4** · **SUP-3** (which role sets supplier credit terms; canon 28 names none) · **RET-1** (legal-retention period; counsel required) · **FIN-7** + **VOID-1** + **VERIFY-1** (unimplemented finance capabilities whose columns already exist; an Invoice state machine would be NEW canon) · **TRANS-1** (classify the 13 ungoverned status columns against canon 26 first) · **DELIV-1** + **PH8-2** (model the existing lease semantics before adding states) · **PLAN-1** (the three "Limited" ceilings canon leaves undefined — a pricing decision) · **DOC-LC-2** · **DOC-LC-3** · **CANON-26-1** · **LIC-1** (external dependency).

Last Completed: **SUP-2 — a ceiling you may not READ is a ceiling you may not SET (2026-09-02, `202607059600`), deployed, parity re-proven.** Batch-6 table-by-table audit, next slice. SUP-1 withheld `suppliers.credit_limit_amount` behind VIEW_FINANCIAL_DOCUMENTS; its WRITE cost only ASSIGN_SUPPLIER, and those role sets differ by **three roles** (branch_manager, department_manager, senior_employee) who could therefore SET the figure they are refused on read — and an actor who sets a value knows it, so the read gate withheld nothing from them. Reproduced as `senior_employee` on **both doors** (direct DML `UPDATE 1`; `create_supplier` at 500,000) with the refusal proven first in the same session. It survived two packages because SEC-1c proved only a *trainee* refused. Guard: `90_supplier_credit_write_authority_test.sql` (15) + 9 HTTP, both with defect injection. **SUP-3 recorded, NOT decided** — whether finance_manager should hold ASSIGN_SUPPLIER is a business rule. Narrative: `session-2026-09-02-supplier-credit-write-authority.md`.
