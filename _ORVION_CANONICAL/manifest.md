...
Live state: **repository, local stack and Primary `vrvtsxexkiiiivlkdxzp` all at 165 migrations** (latest `202607057600`), ledger fingerprint `6f6595de1f1d3d784457e6c60d882fd7` and full-function-surface hash `b511e0edeeec052514fead7ddea5e0ba` (233 functions) identical on both; **75 tables**; 71/601 catalog; **157 `app` functions**; **76 `public` functions = 71 client RPCs + `moddatetime` + 4 service_role-only**, plus **8 reporting views**; **122 policies**; **235 triggers**; **3 pg_cron jobs**; **1 Edge Function (`storage-executor`, ACTIVE)**. Storage: 1 private bucket, 2 policies, 0 objects. n8n: **0 workflows**, 2 credentials. Every figure re-read live from BOTH environments 2026-08-29; **parity PROVEN, Primary's values read from Primary rather than supplied to the guard** (GUARD-1). Primary holds **zero business rows**.
...
Last Completed: **SEC-1 evaluated + API-3 (2026-08-29, `202607057500`/`202607057600`).** **SEC-1: ACCEPT WITH REFINEMENT, awaiting owner ratification** — the proposed model is what ORVION already converged on (54 writable tables: 35 capability-triggered, 15 permission-gated in RLS, 1 integrity-triggered, 3 intentional). Clause 3 needs a DECISION PROCEDURE: **a rule comparing an aggregate ACROSS ROWS cannot be a CHECK, so it lives in one function unless extracted.** That found three — **FIN-8** (done), **FIN-10** (1300 allocated against a 1000 invoice), **QUO-1** (a quotation reading 1000 while its items summed to 6000). Refinement 2: integrity constraints must NOT carry the session-less exemption authorization guards carry. **GOV-7** makes CUST-1's consumer-impact rule durable in `AGENTS.md §3 5b` + a pinned catalog-consumer set. **FIN-11** recorded. Narrative: `session-2026-08-29-sec1-and-the-set-level-class.md`.

Next capability: **executable without the owner** — continue **API-3** (25 left), then Batch 6's remaining items (see the plan). **Needs the owner:** **SEC-1**'s write-path architecture — largest open item, part of the Phase-8 gate — plus DOC-EXP-1, **FIN-7**, SCHED-1, RET-1, RET-2, AUTH-1, FIN-5, SYSADMIN-1, VOID-1, SPP-3, PH8-2, TRANS-1, and releasing the Foundation Completion gate itself. The n8n workflow stays GATED; **CONV-3** records what the integration phase must add. Suite **73 files / 899 assertions**, plus **259 end-to-end HTTP assertions** across six scripts — proven **order-independent**, green on a fresh `db reset` and again under every suite's residue, and (since PAR-2) proven to leave the function surface **byte-equal to Primary** afterwards. Security posture is proven test-by-test; `MASTER_GAP_REGISTER.md` names the file for each.
