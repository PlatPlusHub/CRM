...
1. **Verify the n8n MCP tools are actually loaded in your session** (not merely that `claude mcp list` says Connected — tools are absent in any session that added the server mid-run). If absent, stop: a fresh session is required, nothing else.
