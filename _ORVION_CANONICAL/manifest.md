...
Last Completed: **SEC-1c + SUP-1 — the two halves of the supplier credit question (2026-09-01, `202607059100`/`202607059200`), both DEPLOYED, parity CLEAN.** **SEC-1c (High):** `guard_write_capability` was BEFORE INSERT ONLY on 13 tables — the mirror of **SEC-1b**. A `trainee`, rows proven visible and its own INSERT refused, rewrote a customer, a passenger, and `suppliers.credit_limit_amount` 1000 -> 999999. UPDATE set DERIVED = object-class permission UNION `status_transitions.permission_key`; a CREATE-only rule was rejected on measured role holdings. **Only HTTP caught the handler rule** (a trainee ASSIGNED a lead may log a call) — pgTAP was fully green. Re-scan **13 -> 0**; `lead_interactions` was a false positive of my own detector (**MEAS-3**). **SUP-1 (Medium):** the ceiling was readable by every tenant user; permission derived from `supplier_balance` (VIEW_FINANCIAL_DOCUMENTS), mechanism derived from `booking_items` column grants. First draft was a silent no-op (column revoke against a table grant). **PD-05** limitation recorded. Tests 85 (14) + 86 (11); Pass A = Pass B **86/1154**; HTTP **371**. Narrative: `session-2026-09-01-sec-1c.md`.
