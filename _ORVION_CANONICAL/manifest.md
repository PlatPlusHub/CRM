...
Next capability: **Simple Acceptance — the first exact-SHA shadow proof.** Push the `SPEC-172` Complete SHA to `orvion-preflight` with `main` unchanged, so `orvion-acceptance` runs a non-empty `origin/main..candidate` range; require terminal success on that exact SHA, confirm `main` has not moved, fast-forward the SAME SHA to `main`, then require `REMOTE_CERTIFY: READY`. Never sync preflight by an empty push — an empty range has no governing CR and fails closed. **Phase C stays forbidden until that shadow path is terminal green.** Batch 6 Slice 12 resumes after.
