...
Last Completed: **SPEC-131 — governed Task write path (2026-08-21)**, the first delivery of the RPC-1 programme. 35 of 72 tables had no RPC, so for those a direct PostgREST write was the only path — no authorization, no lifecycle, no event. Evidence that this is an implementation gap rather than a design choice: **37 of 69 seeded permissions were enforced nowhere**, mapping exactly onto the entities missing RPCs, and canon 26 already defines every affected lifecycle. Migration `202607050800` adds `create_task` / `assign_task` / `advance_task` per the canonical Task state machine (`overdue` excluded as System-set), dropping unenforced permissions to 34. Guard `supabase/tests/16_task_write_path_test.sql`; suite 16 files / 84 tests PASS. **RPC-1 remains OPEN** — this closes 3 of 37. Detail: `changes/SPEC-131-task-write-path.md`.
