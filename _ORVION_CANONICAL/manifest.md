...
Live state: **repository, local stack and Primary `vrvtsxexkiiiivlkdxzp` all at 126 migrations** (latest `202607053700`), ledger fingerprint `a07d00fbb02a7f5ae39423f5380adaaf` on all three; 72 tables; 68/583 catalog types/values; 71 permissions; 66 feature entitlements; reference data seeded (82 countries / 82 nationalities / 20 languages / 18 currencies); 110 `app` functions; 116 policies. **repo = local = Primary parity HOLDS, re-proven 2026-08-26.** Prove it with `scripts/check_database_parity.ps1` — never from the repository guard, which reads files only and now says so. Primary holds **zero business rows**. n8n: **0 workflows** (live 2026-08-26), 2 credentials.

Active Change Request: None.

Open owner decisions: **SEC-1/RPC-1** (write-path model; identity half closed by SPEC-138, audit-spine half by WP-00). **New SEC-1 evidence 2026-08-26** — three more forgeries of authoritative history proven, left unfixed because each IS the SEC-1 decision: backdated `lead_assignments`, `customers.first_registered_user_id` attribution theft, `offline_conversions` snapshot mutation (`wp-00-event-write-path-integrity-2026-08-26.md §5`). Also open: PH8-2/PH8-3; C4/C5; A3; AUDIT-4; **PLAN-1**; **BLOCKED-3** (who may set `commission_rate`). AUDIT-2/AUDIT-3 RESOLVED. Evidence in `MASTER_GAP_REGISTER.md`.

Last Completed: **SPEC-154-A — scope-aware financial guard (2026-08-27, `202607053700`), EARNED/CLOSED.** `app.guard_booking_item_financials` authorized by ROLE only, so canon 28's `assigned` scope on ENTER_COST/ENTER_SELLING_PRICE could not be honoured and SPEC-154 had to withhold both. The guard now also requires the item to be the caller's — reusing booking_items' own three-column ownership definition from SPEC-137 rather than inventing one — with `has_tenant_wide_read()` (owner/CEO) the single exemption. Applied to every role because `employee` and `senior_employee` have identical scope permissions and canon's Scope column reads `assigned` for the whole row. Both permissions then granted (employee 28→30). EDIT_LOCKED_COST/APPROVE_FINANCE untouched (canon scope `tenant`). Proven in `40_financial_scope_test.sql`. Detail: `spec-154a-scope-aware-financial-guard-2026-08-27.md`.

Next capability: **Foundation Hardening — Zero-Known-Debt (owner directive 2026-08-24). The n8n workflow stays GATED.** Suite 40 files / 407 assertions; 126 migrations; 71 permissions. Security posture (each proven by a named test): branch/department/assigned read scope, RBAC + finance write authority, plan gating, lifecycle + archive enforced against direct DML, employee financial privacy, document privacy, **audit-spine write integrity (WP-00)**. Remaining, in order:

1. **Post-WP-03 discovery DONE 2026-08-26 (`202607053200`)** — found two cross-tenant abort defects (`process_lead_sla`, `map_outcomes_to_conversions` hit the raising gate on other tenants' behalf); fixed, guarded by `36_subscription_gate_system_paths_test.sql`. **WP-01, WP-02, SPEC-154 DONE 2026-08-27.** Next: **WP-04 documents/storage**. Full ordered backlog: `MASTER_EXECUTION_PLAN.md` Batch 6.
