...
Last Completed: **SPEC-133 - seed geo reference data (2026-08-21)**, resolving REF-1. `countries`, `nationalities` and `languages` were FK-referenced tables holding zero rows, so four columns could only ever be null. Migration `202607051000` seeds 82 countries, 82 nationalities and 20 languages, curated per the SPEC-074 precedent. It also decides the open nationality-vocabulary question on evidence: nationalities reuse ISO 3166-1 alpha-2 codes with demonyms as labels, so nationality and passport-issuing country are directly comparable. Guard `supabase/tests/18_reference_data_test.sql`; suite 18 files / 109 tests PASS. Detail: `changes/SPEC-133-seed-geo-reference-data.md`.
