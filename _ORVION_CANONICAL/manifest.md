...
Current Module: Phase-8 offline-conversion engine — **ORVION-side pipeline IMPLEMENTED and production database deployed**. Local migration state (88 migrations) now leads remote, which matches local only through `202607049600` — the 3 SPEC-120 remediation migrations (`202607049700`/`202607049800`/`202607049900`) have not yet been deployed to production; smoke contract against local is 72 tables, 67 catalog types, and 569 catalog values (production still reflects the pre-remediation 565 until deployed). Remaining Phase-8 item is outside the repository: n8n workflow activation after owner-exclusive Google OAuth `datamanager` credentials and the `orvion_integration` password are provisioned (`MASTER_INTEGRATION_CATALOG.md §3`).
...
Last Completed: SPEC-120 audit remediation — a full-repository audit (2026-08-10, treating `supabase/migrations/**` as ground truth) found two live regressions predating this baseline: `app.merge_customer_identity` and `app.advance_refund` (Phases 4/6, both previously marked Complete) emitted `event_type` codes never registered in the catalog, so every call aborted once `202607049100` hardened `app.record_event`. Fixed (`202607049700`/`202607049800`; RPC corrected to canon's `customer_identity_merged` name, refund's canonical vocabulary registered in canon 27 + catalog); `subscription_plans` seeded (`202607049900`, was never seeded by any prior migration); permanent pgTAP guard added (`supabase/tests/07_event_vocabulary_registry_test.sql`) against this defect class, mutation-tested. Clean `db reset` (88 migrations), full smoke test (72 tables, 67/569 catalog), and all 9 pgTAP assertions verified green; both RPCs re-tested live end-to-end. `feature_entitlements` seeding remains an open owner decision (ambiguous mapping from canon — see `changes/SPEC-120-*.md` Notes and `reports/future-backlog.md`), not implemented. Remote production deployment (last synced through `202607049600`) has not yet received this fix — deploying it is a prerequisite before treating production as consistent with this audit.
