...
Current Module: Phase-8 offline-conversion engine — **ORVION-side pipeline IMPLEMENTED and production database synchronized**. SPEC-120 remediation migrations (`202607049700`/`202607049800`/`202607049900`) were deployed and re-verified on 2026-08-10: local and remote migration histories match through `202607049900`; the verified smoke contract is 72 tables, 67 catalog types, and 569 catalog values. Remaining Phase-8 item is outside the repository: n8n workflow activation after owner-exclusive Google OAuth `datamanager` credentials and the `orvion_integration` password are provisioned (`MASTER_INTEGRATION_CATALOG.md §3`).
...
Last Completed: SPEC-121 status-vocabulary registry guard (2026-08-10), following SPEC-120 audit remediation. SPEC-120 corrected the canonical customer-merge event, registered four refund lifecycle events, seeded the three subscription plans, and added the event-vocabulary guard; SPEC-121 added the corresponding status-transition guard. Both were locally verified (clean `db reset`, smoke contract 72 tables / 67 catalog types / 569 catalog values, full 8-file pgTAP suite) and SPEC-120's three production migrations were deployed and re-verified: local and remote histories match through `202607049900`. `feature_entitlements` seeding remains an open owner decision (ambiguous mapping from canon — see `changes/SPEC-120-*.md` Notes and `reports/future-backlog.md`), not implemented.
