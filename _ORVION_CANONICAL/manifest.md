...
Active Change Request: changes/SPEC-200-draft-baseline-and-approval-replay.md
...
Last Completed: **SPEC-198 pre-approval applicability derivation, Complete (2026-09-19).** Pre-Approval evidence applicability derived from a CONTROL profile alone, so DATABASE, CI, WORKSTATION and Edge Function contracts were admitted unevaluated while the Gate printed PASS. It now derives from any non-`REPOSITORY` profile, a control surface, `supabase/functions/**` or a declared irreversible action, and a genuinely exempt contract reports `NOT APPLICABLE`.
