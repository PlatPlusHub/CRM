...
Live state: **repository, local stack and Primary `vrvtsxexkiiiivlkdxzp` all at 161 migrations** (latest `202607057200`), ledger fingerprint `03509b0f54d600f4ed57fc867e5331bc` and full-function-surface hash `d98abbdd9aea724630f2d97f91a21b08` (230 functions) identical on both; **75 tables**; 71/601 catalog; **154 `app` functions**; **76 `public` functions = 71 client RPCs + `moddatetime` + 4 service_role-only**, plus **8 reporting views**; **122 policies**; **230 triggers**; **3 pg_cron jobs**; **1 Edge Function (`storage-executor`, ACTIVE)**. Storage: 1 private bucket, 2 policies, 0 objects. n8n: **0 workflows**, 2 credentials. Every figure re-read live from BOTH environments 2026-08-29; **parity PROVEN, Primary's values read from Primary rather than supplied to the guard** (GUARD-1). Primary holds **zero business rows**.
...
Last Completed: **DOC-LC-1 — canon 26's Document Lifecycle machine wired at last (2026-08-29, `202607057200`).** `app.status_transitions` held **zero** rows for `documents`, and `enforce_archive_authority` returns early unless `is_archived` changes — it watches the BOOLEAN, so `lifecycle_status_code` was governed only by the catalog check. REPRODUCED: a `trainee` with ARCHIVE_DOCUMENT=false, proven to SEE the document first, was refused by the RPC and **succeeded by direct DML in the same transaction** — and since both write paths read `archived` as a refusal, any tenant user could **permanently freeze a colleague's passport**. No role gained or lost authority. **DOC-LC-2** and **DOC-LC-3** recorded, not guessed. **API-3 30 → 29**. Narrative: `session-2026-08-29-document-lifecycle.md`.

Next capability: **executable without the owner** — continue **API-3** (29 left; `create_journal_entry`, `merge_customer_identity` next), then Batch 6's remaining items (see the plan). **Needs the owner:** **SEC-1**'s write-path architecture — largest open item, part of the Phase-8 gate — plus DOC-EXP-1, **FIN-7**, SCHED-1, RET-1, RET-2, AUTH-1, FIN-5, SYSADMIN-1, VOID-1, SPP-3, PH8-2, TRANS-1, and releasing the Foundation Completion gate itself. The n8n workflow stays GATED; **CONV-3** records what the integration phase must add. Suite **69 files / 835 assertions**, plus **243 end-to-end HTTP assertions** across six scripts — proven **order-independent**, green on a fresh `db reset` and again under every suite's residue, and (since PAR-2) proven to leave the function surface **byte-equal to Primary** afterwards. Security posture is proven test-by-test; `MASTER_GAP_REGISTER.md` names the file for each.
