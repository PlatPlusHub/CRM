...
Active Change Request: changes/SPEC-207-grant-parity-ownership-boundary.md
...
Last Completed: **Batch 6 Slice 12 — membership authority and audit on `public.users`, certified by SPEC-206 (2026-09-20).** The only canon-34 identity-and-access surface with no emitter, and its RLS charged `has_permission` where the RPC charged `authorize`: at `aal1` the RPC refused `42501` while direct DML created, deactivated and re-bound memberships silently. `20260920120000` (deployed under SPEC-203) adds a BEFORE guard and an AFTER emitter, leaving one producer. 118 files / 1958 assertions.
