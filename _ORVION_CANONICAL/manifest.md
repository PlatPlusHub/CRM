...
Active Change Request: `changes/SPEC-194-a-membership-change-costs-the-same-through-every-door.md`
...
Last Completed: **SPEC-192 the mutation oracle must be portable, Complete (2026-09-17).** AUD-01c: Check 12's population is Git's repository-candidate set, so a mandatory `db reset`'s gitignored cache is no longer judged as authored evidence; the date invariant is untouched. Its mutation discriminator is a non-hidden ignored fixture proven on Windows and Linux. Supersedes cancelled `SPEC-191`. Detail in `changes/SPEC-192-the-mutation-oracle-must-be-portable.md`.
