...
Last Completed: **`CTRL-2` closed — Pre-Approval evidence applicability is derived PER PREDICATE, certified by SPEC-210 (2026-09-20).** One trigger answered three questions; both errors were reproduced — an `AGENTS.md` contract forced to invent nine permanent-control obligations, and `scripts/parity_surface.sql`, the parity detector's own measurement surface, reporting `NOT APPLICABLE` after SPEC-207 wrote 68 unread lines. Applicable files 375→397: exactly the 22 verification and publication authorities, `Profiles` unchanged. Write closure now refuses SPEC-203's Write Scope at Approval, not after its Primary deploy. `294 passed, 0 failed`.
