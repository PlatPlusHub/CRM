...
Current Module: Phase-8 offline-conversion engine — **ORVION-side pipeline IMPLEMENTED; production database deployment NOT synchronized.** Corrected 2026-08-10 (handover audit, direct Supabase MCP query against the linked production project `orvion-core`, ref `brplkqmbzffpxqgkkdzo`): its migration history matches local only through `202607049600` and lacks `202607049700`/`202607049800`/`202607049900` entirely; live data confirms it — `catalog_values = 565` (not 569), `subscription_plans = 0` rows, `refund_approved` absent from `event_type`. A prior commit (`5e21aac`, 2026-08-10) had asserted these three migrations were deployed and re-verified; that assertion was checked against the actual production project and found false — corrected here, not merely reworded. SPEC-120 (and therefore the events/catalog/subscription-plan fixes it contains) remains local-only until deployed. Remaining Phase-8 item is outside the repository: n8n workflow activation after owner-exclusive Google OAuth `datamanager` credentials and the `orvion_integration` password are provisioned (`MASTER_INTEGRATION_CATALOG.md §3`).
...
Last Completed: SPEC-121 status-vocabulary registry guard (2026-08-10), following SPEC-120 audit remediation. SPEC-120 corrected the canonical customer-merge event, registered four refund lifecycle events, seeded the three subscription plans, and added the event-vocabulary guard; SPEC-121 added the corresponding status-transition guard. Both fully verified locally (clean `db reset`, smoke contract 72 tables / 67 catalog types / 569 catalog values, full 8-file pgTAP suite). **Production deployment is still outstanding** — see Current Module above. `feature_entitlements` seeding remains an open owner decision (ambiguous mapping from canon — see `changes/SPEC-120-*.md` Notes and `reports/future-backlog.md`), not implemented.
