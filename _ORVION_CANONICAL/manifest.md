...
Last Completed: **SPEC-198 pre-approval applicability derivation, Complete (2026-09-19).** `Evaluate-PreApprovalEvidence` derived applicability from a CONTROL profile alone, so a DATABASE, CI, WORKSTATION or Edge Function contract had its evidence skipped while the Gate still printed `APPROVAL_EVIDENCE: PASS` — reproduced on the real Draft-to-Approved path. Applicability now derives from any non-`REPOSITORY` profile, a control surface, `supabase/functions/**`, or a declared irreversible action, and a genuinely exempt contract reports `NOT APPLICABLE` rather than asserting PASS. Control suite 245 assertions, four isolated mutation kills.
