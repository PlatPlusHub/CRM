...
Last Completed: **PAY-1 / JE-1 / DEV-1 — the finance periphery (2026-09-01), `202607059500`, DEPLOYED.** The Batch-6 slice of tables `authenticated` can write that **no test had ever been about**. **PAY-1 (High):** `app.record_payment` refuses a payment against a draft, voided or archived invoice; the table door did not, so 1,000 EGP sat allocated against a **voided** invoice and every balance derived from allocations misstated what the customer owed — FIN-10's ceiling was green throughout because it caps the AMOUNT and never reads the invoice's STATE. **JE-1:** a journal line posted to a **retired** chart account. **DEV-1:** two rows for one device, reproduced **through the RPC alone** with two concurrent sessions (LIC-2's check-then-act shape); severity Low because `app.mfa_satisfied()` was measured to read only the JWT `aal` claim. **And a defect in my own guard:** PARENT-1's detector could not see PAY-1, because it derived its population from `app.status_transitions` and `invoices` has no rows there (FIN-7). Widened, counterexample-tested, and its residual stated rather than hidden. Narrative: `session-2026-09-01-finance-periphery.md`.
