...
Live state: **repository, local stack and Primary `vrvtsxexkiiiivlkdxzp` all at 123 migrations** (latest `202607053400`), ledger fingerprint `91d8e8b59380caa4e4f046288d6298f0` on all three; 72 tables; 68/583 catalog types/values; 71 permissions; 66 feature entitlements; reference data seeded (82 countries / 82 nationalities / 20 languages / 18 currencies); 109 `app` functions; 116 policies. **repo = local = Primary parity HOLDS, re-proven 2026-08-26.** Prove it with `scripts/check_database_parity.ps1` — never from the repository guard, which reads files only and now says so. Primary holds **zero business rows**. n8n: **0 workflows** (live 2026-08-26), 2 credentials.
...
Last Completed: **WP-02 / SPEC-153 — Class A events (2026-08-27, `202607053400`), EARNED/CLOSED.** Five events whose producer existed but emitted nothing: `payment_allocation_created`, `trusted_device_revoked`, `trusted_device_reverified`, `document_superseded`, `user_branch_transfer_completed` (`assign_user_branch` left no audit trace at all). Emitted by trigger so direct DML cannot skip them. **`payment_allocation` needed a new `events` read-policy dispatch branch**: `has_tenant_wide_read()` is `VIEW_ALL_BRANCHES` (ceo/owner only), so the event would have been invisible to `finance_manager`. Proven in `38_class_a_events_test.sql`. Detail: `wp-02-class-a-events-2026-08-27.md`.

Next capability: **Foundation Hardening — Zero-Known-Debt (owner directive 2026-08-24). The n8n workflow stays GATED.** Suite 38 files / 376 assertions; 123 migrations; 71 permissions. Security posture (each proven by a named test): branch/department/assigned read scope, RBAC + finance write authority, plan gating, lifecycle + archive enforced against direct DML, employee financial privacy, document privacy, **audit-spine write integrity (WP-00)**. Remaining, in order:

1. **Post-WP-03 discovery DONE 2026-08-26 (`202607053200`)** — found two cross-tenant abort defects (`process_lead_sla`, `map_outcomes_to_conversions` hit the raising gate on other tenants' behalf); fixed, guarded by `36_subscription_gate_system_paths_test.sql`. **WP-01 + WP-02 DONE 2026-08-27.** Next: 58 never-emitted events remain, all Class B/C per SPEC-153 — none has a producer, so the next package is elsewhere.
