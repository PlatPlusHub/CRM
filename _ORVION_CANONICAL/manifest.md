...
Last Completed: **SPEC-127 — enforce catalog codes at write (2026-08-21)**, from the second (CRM-coherence) health gate. The catalog was canonical after SPEC-126 but not authoritative where it is consumed: 35 of 72 tables have no RPC write path, so `tasks` accepted `task_type_code='TOTALLY_MADE_UP'`, `suppliers` accepted `MADE_UP_SUPPLIER` and `conversations` accepted `carrier_pigeon`. Migration `202607050400` adds `app.enforce_catalog_codes`, a trigger on the 12 ungoverned tables — the enforcement ADR-0006 explicitly leaves optional per column. It also narrows CAT-4: deactivated values can no longer be selected for new records, while historical rows stay editable. Guard `supabase/tests/12_catalog_code_enforcement_test.sql`; suite 12 files / 54 tests PASS. Detail: `changes/SPEC-127-enforce-catalog-codes-at-write.md`.
