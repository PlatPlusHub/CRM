...
Last Completed: **Four orphaned migrations recovered from Primary, and the authorization refactor nobody could see (2026-09-03).** Primary held **188** migrations while this repository held **184** — `202607059600`–`202607059900` (SUP-2, SUP-3, RBAC-5, SUP-4a) were applied 2026-09-02 and **never committed**; commit `1193643`, cited as their home, is not an object in this repository (**RECOVER-1**, Critical). Recovered byte-identical by md5 from Primary's ledger and replayed by a clean `db reset`. Their tests, ADR and register rows were NOT recoverable and are rewritten, not restored: **ADR-0027**, `90_`/`91_` (31 assertions), 19 new HTTP assertions. **RBAC-6** fixed a defect found while doing so — `app.effective_permissions` had no `public` wrapper, so the dashboard capability was unreachable (`202607060000`). **`CREDIT LIMIT ENFORCED = NO`**; everything derivable shipped, the residue is **SUP-4b**. Narrative: `session-2026-09-03-orphaned-migration-recovery.md`.
