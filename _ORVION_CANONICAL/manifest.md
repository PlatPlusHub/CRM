...
Last Completed: **SPEC-135 - approval-request subject integrity (2026-08-21)**, resolving REL-2. `approval_requests` carried a polymorphic subject and typed FKs with nothing forcing them to agree, while `review_finance_approval` joins on `booking_item_id` - so a divergent row would have had the reviewer act on one item while the audit trail named another. Canon 31 settles authority: it marks the optional fields explicitly and lists the subject pair without that marker, so the pair is authoritative and the typed columns are accessors constrained to agree. Also closed a canon drift (the subject columns were nullable while canon lists them as core). Migration `202607051200`; guard `supabase/tests/20_approval_subject_integrity_test.sql`; suite 20 files / 126 tests PASS. Detail: `changes/SPEC-135-approval-request-subject-integrity.md`.
