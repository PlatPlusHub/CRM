...
Last Completed: **SPEC-173 certified evidence names its context, Complete (2026-09-12).** `-Certify` asked GitHub for a SHA and nothing else, so one SHA's preflight qualification and its main promotion were judged as one body of evidence, and every promoted SHA reported FAILED on a correct preflight failure. The target branch is now derived once, persisted in the receipt and read back; the query names SHA + branch + `push`, bounded; every row is revalidated on `headSha`/`headBranch`/`event`, a contradicting row discarded and named; READY also requires the live remote target ref to still equal the certified SHA (`TARGET_REF_MOVED`). The acceptance boundary is frozen at the proven environment. Suite 143 → 154; four mutants each killed exactly one assertion.
...
Next capability: **Simple Acceptance — Phase C readiness review (owner).** Phase B closes once a promoted SHA earns `REMOTE_CERTIFY: READY`. Phase C is then one minimal Ruleset change: add `orvion-acceptance` as the single required check bound to the GitHub Actions App, preserving `non_fast_forward`, `deletion` and zero bypass, then read it back. Never sync preflight by an empty push — an empty range has no governing CR and fails closed. A rejected candidate is replaced on preflight only by an explicit expected-value lease, never plain force; that Publisher is a separate unit. Migration CI is OBSERVE ONLY until the first Batch-6 DB slice compares it. Batch 6 Slice 12 resumes after.
