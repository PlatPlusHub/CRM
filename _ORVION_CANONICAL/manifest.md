...
Last Completed: **COLD-2 — the handoff field no guard compared (2026-09-02). Repository-only — no migration, no database change.** `ai-map.json`'s `live_state.active_change_request` is the field `AGENTS.md §4` BRANCHES on and the whole of the agent handoff (`§6`, `CR_LIFECYCLE.md §9`), and Check 7 never compared it. Proven blind in **both** directions first: ai-map naming a SPEC that does not exist, and the manifest naming a SPEC ai-map did not carry, each printed CLEAN. Check 7 now compares it by value on the identical contract as `Last Completed` — whose own comparison had been inert for forty commits until 4b67d3f and was never recorded until now. Narrative: `session-2026-09-02-cold-start-handoff-guard.md`.
