...
Active Change Request: `changes/SPEC-165-range-completion-of-a-contract-created-in-range.md`
...
Last Completed: **SPEC-164 Agent Control evidence lifecycle closure, Complete (2026-09-11).** `Complete` now requires a fresh `-Finish` certification receipt fingerprinted to the implementation state; `DATABASE` executes the `ENGINEERING_METHOD.md §4` protocol instead of listing it, and a declared `supabase-primary` runs `check_primary_ledger.ps1`; `-Certify` proves the EXPECTED workflow set against the exact SHA. Control suite 94 → 117 assertions. Product and schema untouched. The manifest pointer and contract Status became one invariant checked in both directions; capabilities carry an evidence class and `WORKSTATION` runs its doctor instead of deferring it; `-Certify` owns POST_PUSH evidence against the exact pushed SHA; five semantic pointer drifts were repaired and Checks 26 and 27 prevent their return; `AGENTS.md` fell from 29,766 to 15,739 bytes with its owner-ratified method relocated verbatim to `ENGINEERING_METHOD.md`. The `pull_request` path is proven and ruleset `main integrity` blocks force-push and deletion. Product work was not changed.
