...
Last Completed: **SUP-4b — the supplier credit ceiling WARNS and never refuses** (2026-09-04, `202607060100`, deployed to Primary; **ADR-0028**). AFTER-row probes on the only two tables that move exposure, comparison **in the ceiling's own currency**, one notification each to the **Company Owner and Finance Manager** by ROLE — who is TOLD, not who is ALLOWED; authorization untouched. Idempotency is the **event ledger**, not a new column or status vocabulary. `supplier_credit` also returns `exposure_amount`/`threshold_exceeded`. **No email is sent** — the obligation is recorded `pending`; canon 10 states the boundary. Its four sibling targets (TRANS-1, DELIV-1/PH8-2, PLAN-1, QUO-4) are owner decisions held at their boundaries; residues **SUP-4c**, **CUST-3**. Detail: `session-2026-09-04-owner-policy-package.md`. **Since then one read-only session shipped no capability**: `python3` is a Microsoft Store alias stub while Python 3.12.10 is healthy as `python`, and ORVION references `python3` nowhere — nothing fixed because nothing was broken; no migration, no database contact. Narrative: `session-2026-09-04-python3-environment.md`.
