...
Last Completed: **The cold-start contradiction, and Checks 16–17 (2026-09-01). Repository-only — no migration, no database change.** `32_execution_roadmap.md` named **SEC-1** an open owner decision blocking Phase 10 four days after the owner ratified it (OWNER-1), and restated a stale endpoint count inside the very sentence declaring it never restates that count. Verifying the first exposed a third: the register's **own SEC-1 row** still said AWAITING OWNER RATIFICATION while its OWNER-1 row recorded the ratification — **Check 2 could see neither**, because a status cell opening `**EVALUATED …**` is neither its OPEN form nor its resolved form, and no guard had ever read canon against the decision list. Both numbers **deleted rather than refreshed**. **Check 16** judges canon list-item claims against the manifest's own open-decision line (no closed-ID list is maintained anywhere); **Check 17** forbids canon restating the generated endpoint count. Attacked with 13 cases, including a second-direction test that mutates the *authority* and leaves the consumer untouched. Prior capability: PAY-1/JE-1/DEV-1, `202607059500`, deployed. Narrative: `session-2026-09-01-cold-start-guard.md`.
