...
Last Completed: **STALE-1 — freshness metadata becomes a guarded invariant (2026-09-05).** No migration; the database is untouched. A current-state document's `Last updated:` line could be older than the document's own content while every guard reported CLEAN — the class had been hand-repaired four times since 2026-07-14 and had never produced a check. **Check 21** now fails the build on it, on a **semantic** invariant, never a wall-clock one: a header may not be older than the newest date its own body carries, so a finished document passes forever. Generation from git was rejected — the field carries a written annotation git cannot supply. Its first run found **four** stale Masters, not the one reported. Registered as `GOVERNANCE.md §6` rule 9 (v1.13) and **Docs-21**. Also: **ADR-0027/0028** restored to the ARB overlay, the sweep scope corrected **75 → 77** against the catalog, and **Batch 6 given exit criteria EC-1…EC-11 — still NOT complete.** Narrative: `session-2026-09-05-freshness-invariant.md`.
