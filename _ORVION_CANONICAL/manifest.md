...
Live state: **repository, local stack and Primary `vrvtsxexkiiiivlkdxzp` all at 125 migrations** (latest `202607053600`), ledger fingerprint `c577ad39945bf58dfb8b21b5bdea894f` on all three; 72 tables; 68/583 catalog types/values; 71 permissions; 66 feature entitlements; reference data seeded (82 countries / 82 nationalities / 20 languages / 18 currencies); 109 `app` functions; 116 policies. **repo = local = Primary parity HOLDS, re-proven 2026-08-26.** Prove it with `scripts/check_database_parity.ps1` — never from the repository guard, which reads files only and now says so. Primary holds **zero business rows**. n8n: **0 workflows** (live 2026-08-26), 2 credentials.
...
Last Completed: **SPEC-154 — employee role canon alignment (2026-08-27, `202607053500`+`202607053600`), EARNED/CLOSED.** Canon 28 marks 17 permissions `Yes`/`Assigned only` for Employee; the seed stopped at `senior_employee`, so an ordinary employee could not quote, book, close a lead or send a message. 15 granted (13→28); scope still enforced by the SPEC-137 RLS model. Withheld on evidence: `ENTER_COST`/`ENTER_SELLING_PRICE` (guard is role-based only — granting would let an employee price a COLLEAGUE's item; caught by test 29 and withdrawn rather than weakening the guard) and `VIEW_FINANCIAL_DOCUMENTS` (binary tenant-wide gate). Also fixed **`create_booking`, broken on the direct path for every role**. Proven in `39_employee_day_one_workflow_test.sql`. Detail: `spec-154-employee-role-canon-alignment-2026-08-27.md`.

Next capability: **Foundation Hardening — Zero-Known-Debt (owner directive 2026-08-24). The n8n workflow stays GATED.** Suite 39 files / 392 assertions; 125 migrations; 71 permissions. Security posture (each proven by a named test): branch/department/assigned read scope, RBAC + finance write authority, plan gating, lifecycle + archive enforced against direct DML, employee financial privacy, document privacy, **audit-spine write integrity (WP-00)**. Remaining, in order:

1. **Post-WP-03 discovery DONE 2026-08-26 (`202607053200`)** — found two cross-tenant abort defects (`process_lead_sla`, `map_outcomes_to_conversions` hit the raising gate on other tenants' behalf); fixed, guarded by `36_subscription_gate_system_paths_test.sql`. **WP-01, WP-02, SPEC-154 DONE 2026-08-27.** Next: **SPEC-154-A** (scope-aware financial guard). Full ordered backlog: `MASTER_EXECUTION_PLAN.md` Batch 6.
