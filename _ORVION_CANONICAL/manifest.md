...
Active Change Request: `changes/SPEC-173-certified-evidence-names-its-context.md`
...
Last Completed: **SPEC-172 acceptance carries the evidence it replaces, Complete (2026-09-12).** The admission boundary's cleanup ran `npx supabase stop` under `if: always()`; on a run that failed before `npm ci` it fetched supabase 2.117.0 against a lockfile pinning 2.109.0 and reported success — a failure path installing software the repository never chose. Every `npx supabase` is now `node_modules/.bin/supabase`, and cleanup invokes nothing when it is absent, so `npm ci` is the one installation authority. The four guard-calibration suites now run inside the one acceptance job, before `npm ci`, so the intended required check is no longer weaker than the union it replaces. Suite 140 → 143, the new assertions evaluating false against the workflow as it shipped.
...
Next capability: **Simple Acceptance — context-correct certification, then Phase C.** The first exact-SHA shadow proof is COMPLETE: `18abbea` passed `orvion-acceptance` on `orvion-preflight` and was promoted unchanged to `main`, so `preflight == main` holds. `SPEC-173` repairs `-Certify`, which judges a SHA without naming branch or event and so reports FAILED on a promotion whose every `main` run is green. Never sync preflight by an empty push — an empty range has no governing CR and fails closed. **Phase C stays forbidden until `REMOTE_CERTIFY: READY` is earned on a promoted SHA.** Batch 6 Slice 12 resumes after.
