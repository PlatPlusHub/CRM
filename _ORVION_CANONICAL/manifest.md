...
Last Completed: **WP-01 — creation-event completeness (2026-08-27, `202607053300`), EARNED/CLOSED.** Four registered, active `*_created` types had a producer and were never emitted, so timelines began halfway through the relationship. Emitted by AFTER INSERT trigger, not inside the RPCs: SEC-1 is open, so a direct write would otherwise create an entity with no creation event — and the trigger also makes the upsert-shaped `record_trusted_device` fire once instead of on every re-login. Cross-path sweep clean: only the four producers INSERT these tables, and `map_outcomes_to_conversions` does not consume the new types. Proven in `37_creation_event_completeness_test.sql`. Detail: `wp-01-creation-events-2026-08-27.md`.
