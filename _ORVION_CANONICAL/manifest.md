...
Active Change Request: `changes/SPEC-178-repository-dates-are-culture-invariant.md`
...
Last Completed: **SPEC-180 certification target is the upstream, Complete (2026-09-14).** The certification receipt recorded the branch the agent was standing on rather than the branch the work publishes to, so any contract executed off `main` could never reach `REMOTE_CERTIFY: READY`. `Target-Branch` now reads the configured upstream merge ref. Suite 158 -> 161.
