...
Current Module: Phase-8 offline-conversion engine — ORVION-side pipeline IMPLEMENTED and confirmed synchronized across BOTH authoritative Supabase projects (full comparison 2026-08-10). **ORVION has TWO authoritative Supabase projects that must stay synchronized — full topology + safety rules: `MASTER_INTEGRATION_CATALOG.md §0`.** SPEC-120 deployed, migration-version-reconciled, and byte-for-byte schema/data-identical on both primary (`vrvtsxexkiiiivlkdxzp`, via the `supabase-primary` MCP server) and secondary/mirror (`orvion-core`, ref `brplkqmbzffpxqgkkdzo`, via `claude_ai_Supabase`): 88/88 migrations match; full object audit (columns/constraints/indexes/RLS/policies/functions/triggers/views/sequences/enums) identical; 569 catalog values, 3 subscription plans, all 4 refund events present, `customer_merged` absent — on both. Remaining Phase-8 item is outside the repository: n8n workflow activation after owner-exclusive Google OAuth `datamanager` credentials and the `orvion_integration` password are provisioned (`MASTER_INTEGRATION_CATALOG.md §3`). Google Ads Data Manager technical validation (account/conversion-action USER-CONFIRMED, transactionId strategy recommended, one `validateOnly` test pending execution) is tracked in `MASTER_INTEGRATION_CATALOG.md §4`; n8n's production Supabase target is USER-CONFIRMED as Primary (`§0`).
