...
Last Completed: **SPEC-168 shadow acceptance workflow, Complete (2026-09-12).** `orvion-acceptance.yml` produces the admission evidence: one job, one check identity, `push:` to `orvion-preflight` only, no path filters, base from `origin/main` with a fail-closed ancestry test, the Gate over the candidate range, Supabase from `package-lock.json` via `npm ci` rather than a second version authority. Prerequisite repaired in the same change: `Workflow-Expectations` read every item under `push:` as a path glob, so an inline `branches: [x]` left a workflow looking unfiltered and expected on every push, which would make `-Certify` fail closed forever on a run that can never happen. `Trigger-List` parses both YAML styles; branch filters settle before path filters. Ref `orvion-preflight` created at `e99324e`; no Ruleset change yet. Suite 127 → 132 assertions.
