...
Active Change Request: changes/SPEC-198-preapproval-applicability-derivation.md
...
Last Completed: **SPEC-196 pre-approval evidence sufficiency and SPEC identity allocation, Complete (2026-09-19).** Approval is admissible only on bounded evidence, with applicability derived from repository evidence so a declared `Routine` cannot exempt control work and `INDETERMINATE` blocking as firmly as `FAIL`. Identity separates allocation, monotonic reservation (which drives candidate advancement, not only refusal) and origination; enforcement activates on the declared `SPEC Allocation Enforcement` marker, never a diagnostic string. Control suite 174 → 231 assertions, 21 mutation kills. `CTRL-1` OPEN.
