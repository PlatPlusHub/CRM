...
select is(
  ...
  'booking_items.operational_owner_user_id, booking_items.owner_user_id, booking_items.sales_owner_user_id, bookings.owner_user_id, complaints.owner_user_id, customers.last_interaction_user_id, journal_entries.voided_by, lead_assignments.assigned_user_id, leads.assigned_user_id, leads.owner_user_id, payments.verified_by, quotations.owner_user_id, service_requests.owner_user_id, tasks.owner_user_id, totp_enrollments.auth_user_id, trusted_devices.auth_user_id, user_branch_assignments.user_id, user_permission_grants.user_id, user_role_assignments.user_id, users.auth_user_id',
  'CLASS GUARD (ATTR-2, structural): every column that FOREIGN-KEYS to a relation named `users` -- public.users AND auth.users, which is why the canon-34 identity columns appear -- on a table `authenticated` can write directly, and that no BEFORE trigger derives, is exactly this classified set. Unlike assertion 22 this asks no question about the column NAME, so an actor column called anything at all appears here. A new entry is not necessarily a defect -- it is an unclassified column, and it must be classified in MASTER_GAP_REGISTER.md before this line is edited. `otp_challenges.auth_user_id` LEFT this set at 202607061600 (Batch 6 slice 6, OTP-1): the join above requires an INSERT or UPDATE grant, and the repair revoked both, so the column is no longer directly writable. A DEPARTURE in this direction is the guard working -- the write surface narrowed.')
from (
